...
fn trusted_git_capture_ignores_replacements_and_rejects_non_regular_modes() {
    ...
    let repository = test_support::unique_path("slice-review-trusted-git");
    ...
    let mut permissions = fs::metadata(repository.join("authority.md"))
        .unwrap()
        .permissions();
    permissions.set_mode(0o755);
    fs::set_permissions(repository.join("authority.md"), permissions).unwrap();
    test_git(&repository, &["add", "authority.md"]);
...
fn trusted_cleanliness_ignores_path_injected_git() {
    ...
    let repository = test_support::unique_path("slice-review-path-injection");
    let fake_bin = test_support::unique_path("slice-review-fake-git");
    fs::create_dir(&repository).unwrap();
    ...
    fs::write(repository.join("untracked"), b"must be observed\n").unwrap();
    ...
    let output = git::trusted_command_in(&repository)
        .env("PATH", &fake_bin)
...
#[cfg(unix)]
fn test_git(repository: &path::Path, arguments: &[&str]) -> String {
    let output = process::Command::new("/usr/bin/git")
        .env_clear()
