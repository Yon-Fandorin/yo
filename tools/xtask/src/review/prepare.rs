...
use crate::{
    bounded_file, git,
    review::{
        egress as review_egress, packet as review_packet,
        target_admission as review_target_admission,
    },
    review_protocol::{digest, relative},
    review_result, slice_contract, slice_worktree,
};
...
fn workflow_code_owner(path: &str) -> Option<&'static str> {
    if review_module(path, "packet")
        || review_module(path, "prepare")
        || review_module(path, "delta")
    ...
    } else if review_module(path, "delivery")
        || review_module(path, "egress")
        || review_module(path, "target_admission")
        || review_module(path, "continuation_preflight")
        || matches!(
            path,
            "tools/xtask/src/review/continuation_preflight.rs"
                | "tools/xtask/src/review/session.rs"
        )
    {
...
fn review_module(path: &str, module: &str) -> bool {
    path.strip_prefix("tools/xtask/src/review/")
        .and_then(|relative| relative.strip_prefix(module))
        .is_some_and(|suffix| suffix == ".rs" || suffix.starts_with('/'))
}
