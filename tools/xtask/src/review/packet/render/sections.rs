...
#[cfg(test)]
pub(in crate::review::packet) fn append_section(
    output: &mut Vec<u8>,
    kind: &str,
    name: &str,
    path: &str,
...
pub(in crate::review::packet) fn encode_section(bytes: &[u8]) -> Vec<u8> {
...
#[cfg(test)]
pub(in crate::review::packet) fn decode_section(bytes: &[u8]) -> Result<Vec<u8>, String> {
...
pub(in crate::review::packet) fn count_tokens(bytes: &[u8]) -> Result<usize, String> {
...
pub(in crate::review::packet) fn require_budget(
    actual: usize,
    maximum: usize,
) -> Result<(), String> {
    if actual <= maximum {
