...
def collect_state(root, report):
    ...
    commits = [json.loads(record["payload"]) for record in records]
    report["accepted_request_records"] = tag_count(commits, "backend_request_accepted")
    report["turn_finished_records"] = tag_count(commits, "turn_finished")
