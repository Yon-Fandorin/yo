...
pub(super) struct LocalTlsChildSpec<'a> {
    pub(super) script: &'a str,
    pub(super) certificate: &'a OsStr,
    pub(super) key: &'a OsStr,
    pub(super) ready: &'a Path,
    pub(super) requests: &'a Path,
    pub(super) accepted: &'a Path,
    pub(super) sent: &'a Path,
    pub(super) mode: &'a str,
    pub(super) closed: &'a Path,
    pub(super) payload: &'a Path,
...
pub(super) fn spawn_local_tls_child(spec: LocalTlsChildSpec<'_>) -> ChildGuard {
...
pub(super) fn wait_for_local_tls_ready(
...
impl ChildGuard {
    pub(super) fn new(child: Child, sensitive_values: Option<Vec<String>>) -> Self {
        let mut child = child;
    ...
    pub(super) fn assert_success(&mut self, message: &str) {
        let status =
...
pub(super) fn child_sensitive_values(
    paths: &[&OsStr],
    location: &str,
    key: &OsStr,
) -> Option<Vec<String>> {
...
pub(super) fn create_private_file(path: &Path) {
...
pub(super) fn set_private_permissions(path: &Path) {
    #[cfg(unix)]
    {
...
pub(super) fn set_directory_permissions(path: &Path) {
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;
