...
| delegated host가 작업을 수락했지만 대화 기록이 갱신되지 않음 | 해당 `backends/delegated-*/events.rs` → `AgentRuntime::poll_event`와 Journal 추가 → agent-session 변경 알림 → `yo-cli/agent` Transcript cursor → `TuiState::observe_record` → Chat transcript |
