...
완료한 real-host 기본·구조화 편집 실험은
[파일 수정 도구 비교](./file-mutation-tools.md)를 참고한다.
...
| `$` trigger, Codex catalog decode, scope filtering, 비활성 행, typed skill 선택 | `cargo test -p yo-tui skill_reference`, `cargo test -p yo-core skill_reference`, `cargo test -p yo-backend-delegated-codex skill_catalog` | `yo-tui/src/prompt/skill_reference`, `yo-core/src/skill_reference`, `backends/delegated-codex/src/skill_catalog` |
...
프로필도 호환된다. 이 승인 중 패널의 `Review proposed files`는 더 최근의 무관한 변경 대신 연결된 활동의
...
파일 변경 블록에서 `Alt+D`를 누르거나 승인 패널의 `Review proposed files`를 선택하면 보존된 타입 지정 FileChange를 읽는 전용 화면이 열린다. Git 작업 트리 스캔은 아니다.
...
파일 전환은 제목부터 시작한다. 접힌 Chat은 큰 diff 요약과 `Alt+D` 안내를 유지하며,
...
snapshot 투영을 보여준다. `item/tool/requestUserInput`은 질문을 순서대로 제시하고 마지막
답변 뒤 원래 JSON-RPC ID로 답변 map을 한 번 전송한다. 비밀이 아닌 선택 번호는 label로
변환하고 직접 쓴 Unicode 답변은 보존한다. 비밀 질문은 선택지·메모 없는 자유 입력이어야
하며 요청 전용 숨김 편집기를 사용한다. 값은 최종 응답을 쓸 때까지 프로세스 메모리에만
둔다. Core runtime 테스트로 후속 요청의 상관관계, 중복 응답, 부분 답변 중단, 늦은
resolved와 값 없는 비밀 영수증을 확인한다. 잘못된 비밀 질문은 입력 전에 닫힌 실패로
끝낸다. 모델용 이미지 입력이나 비밀 값 저장은 추가하지 않는다.
...
상세 변경 화면은 접기를 우회한다. `runner/tests/appearance.rs`는 실제 frame, 도구·diff 독립 설정,
...
검증한다. 기존 diff 색상·접기·폭과 상세 변경 화면의 파일 탐색을 사용한다. 앞의 설명은 별도
...
전달하며 상세 변경 화면의 파일 이동을 위한 명시적 인용 경로 헤더를 제공한다. 부분 갱신은
...
toolCallId로 승인 패널의 `Review proposed files`에 연결한다. 다른 종류와 종료된 호출은 연결하지
...
스냅샷을 먼저 갱신한 뒤 승인을 게시한다. 최신 diff가 이전 상세 변경 화면의 본문을 교체하고
...
순서 있는 QuestionChoice 이름·설명을 전달한다. Codex의 완전한 일반 질문 캡처를 검증해
이 표시 프로필로 투영하며 기존 숫자→이름 응답 변환과 요청 ID가 답변 연결을 결정한다. 선택지 64개와
...
묶인 숨김 편집기로 받는다. 현행 문맥 초안은 비밀 값을 프로세스 메모리에만 두며,
공개 영수증에 값·해시·길이를 남기지 않는다. 고정한 Codex
...
메모 원문과 저널 코덱 왕복을 검증한다. 현행 문맥 초안 흐름에서는 재시작한 비밀 답변을
...
알린다. TUI는 기존 숨김 비밀 편집기를 사용한다. 저장 제안이 없으면 Yo는 값을 프로세스 메모리에만 둔다.
...
공개 저장 제안이 있으면 TUI는 기본값 `Use once`와 별도로 1~365일 또는 삭제할 때까지
보관하는 선택을 제공한다. 정책 선택만으로 제출하지 않고 나중에 Enter를 다시 눌러야 한다.
내구성 저장에는 실제 인증된 계정 식별자와 안전한 로컬 경로가 필요하다. 현재 관리형 경로는
OpenRouter의 인증된 현재 키 조회에서 creator·organization·workspace 식별자를 확인하며,
이 식별자를 얻지 못하면 일회성 입력만 가능하다. 별도 저장소는 고정 크기의 비밀 값을
암호화하고 공개 메타데이터를 인증하며 교체·삭제·만료 정리를 중단 후에도 복구한다.
`/secrets`는 인증된 공개 메타데이터만 표시하고 `/secrets delete <ID>`는 정확히 한 항목을
삭제한다. 나중에 대상이 일치하는 실제 요청에는 `Recovery available`과 `Ctrl-R` 안내를
표시하며, 값을 불러온 뒤에도 제출에는 새 Enter 입력이 필요하다. 위임형 비밀 요청에는 이 저장 제안이 없다.
스키마·바이트 한도, 단독 호출, 전송 전 내구성,
...
거부한다. 공통 TUI는 Codex wire 필드를 읽지 않으며 검증된 백엔드 중립 캡처를 사용한다.
...
초안 복원, 명령 저널 왕복과 최신 화면 확인을 다룬다. 비밀이 아닌 답안의 재시작
복구는 아래 문맥 초안 흐름을 사용하며 provider 요청을 재개하지 않는다. 비밀 답변은
새 live 요청에서 다시 입력해야 하며 종료된 요청을 재개하지 않는다.
...
중간 응답과 이전 형식의 표시 내용은 할당 전에 ToolOutput::MAX_SNAPSHOT_BYTES로 제한한다.
초과하면 표시 생략을 명시하고 원래 응답은 저널에 남는다. 완전한 최종 캡처에는 아래의
1 MiB 한도가 적용된다. 답·메모의 정확한 한도와 첫 초과 바이트를 검증한다.
...
Chat의 `Ctrl+F`와 `/find QUERY`는 보존된 대화 기록에서 확정된 일반 User·Assistant 메시지만 검색한다. 검색은 범위가 제한된 대소문자 무시 문자 그대로의 결과 선택기이며, 알림·도구·추론·구조화된 미디어 snapshot·대기 중인 요청은 제외한다. Enter는 선택한 항목으로 이동하고 검색어와 저장된 초안을 전송하지 않으며, 선택기가 보이는 frame으로 아직 반영되기 전에도 같은 규칙을 따른다. Esc와 Ctrl+C는 초안·커서·참조·이미지·undo 상태를 복원하고, 새 실행 가능한 요청이 오면 Find를 먼저 닫는다. UTF-8 문자가 검색어 byte 제한을 가로지르는 경우, 보이지 않는 선택기, 폭 변경 뒤의 이전 Inline 기록, End로 최신 출력 복귀를 확인한다. 검색 제한과 불완전한 범위는 선택기에서 보여야 한다.
...
별도 재사용 비밀 저장소는 이 파일을 이행하거나 목록에 표시하지 않는다. 현재 경계는 core·TUI 인터뷰
테스트로 정확한 살아 있는 요청 이어 쓰기, 종료된 요청의 읽기 전용
보기, 동일 Session 선택, 명시적 폐기, 내구성 봉인 뒤 삭제와
비밀 배제를 확인한다. 현행 v1 스키마는 Linux 전체 워크스페이스 테스트를
통과했다. 정확한 `9c3f6caa` 후보도 Mac `yo-tui` profile과 격리된 Fullscreen
tmux 흐름을 통과했다. 오프라인 ACP 호스트가 비밀이 아닌 질문을 보내자 Yo가
이를 `UserInputRequest`로 기록했다. 초안 생성·답변 후 durable seal에 따른 삭제·
중단 후 읽기 전용 조회·
명시적 폐기를 확인했다. 이는 Mac Yo/ACP/TUI 경로의 검증이며 실제 모델 서비스
검증은 아니다. 실제 서비스 흐름에는 적절한 설치 호스트가 더 필요하다.
[Mac 인터뷰 증거](terminal-matrix.md#mac-v1-인터뷰-요청-흐름)를 따른다.
