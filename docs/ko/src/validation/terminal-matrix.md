...
인증된 Grok 모델 경로는 구독 용량을 사용했다. 따라서 어느 호스트도 이 정확한
무료 모델 실서비스 여정을 시작할 수 없다. 위의 관리형 QwenCloud 결과를 위임형
호스트 검증으로 간주하지 않는다. 해당 검증을 주장하기 전에 호스트 기능과 무료
사용 자격을 다시 확인해야 한다. 현재 외부 호스트의 선행 기능이 없다.
...
저장소 번들·모델 프롬프트·비밀값을 보내지 않았다. 호스트에 접속할 수 있게 된
것만으로 위임형 실서비스 결과가 바뀌지는 않았으며, 상태는 미검증이다.
