...
| [`model_connector`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/model_connector/mod.rs) | provider 중립 Connector request·observation·취소·limit 어휘. 단계적 추출 동안에는 이후 composition Slice가 최종 port를 도입하고 routing을 전환할 때까지 기존 core 내부 concrete Connector가 제품 runtime owner로 유지된다 | composition 이후의 provider wire ownership 또는 semantic Activity와 tool loop를 맡는 Yo-managed backend |
...
| [`connectors/openai-responses`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/connectors/openai-responses/src/lib.rs) | 독립 검증된 OpenAI Responses request 직렬화와 bounded response-event decoding. 현재 제품 routing은 바꾸지 않고 도입하며 이후 composition Slice가 최종 중립 port에 연결한다 | model 선택, credential, 공통 transport byte 또는 managed semantic Activity |
