...
| [`backends/delegated-codex`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/backends/delegated-codex/src/lib.rs) | `codex app-server` 시작과 protocol 분류, provider ID 연결, core event 변환, continuation evidence를 위한 backend/effective-model/thread identity 보존, ephemeral이 아닌 저장 thread, 최신 durable locator에 대한 검증된 `thread/resume` 한 번, Session 없는 인증형 `account/read`와 bounded paginated visible `model/list` inventory, foundation transport 위에서 worker가 소유하는 `skills/list` metadata catalog | provider 중립 runtime 의미, 다른 delegated host, structured dispatch 전 정확한 skill admission 또는 binding replacement policy |
| [`backends/delegated-grok`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/backends/delegated-grok/src/lib.rs) | `grok agent stdio` 시작, ACP v1 초기화, cached-login 인증, Session 없는 정확한 account와 advertised `modelState` inventory snapshot, 제한된 request·Session 연결, permission mapping, event 변환, `session/load` continuation, foundation transport 위의 결정론적 정리 | provider 중립 runtime 의미, 다른 delegated host, 직접 xAI Provider 접근, 설치된 host가 보고하지 않은 quota window 추론 또는 semantic handoff policy |
