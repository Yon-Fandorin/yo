...
| [`backend/codex`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/backend/codex/mod.rs) | `codex app-server` 생명주기, JSON transport와 protocol 분류, provider ID 연결, core event 변환, worker가 소유하는 `skills/list` metadata catalog | 새 provider 동작을 노출하기 전 `backend/contract.rs`, structured dispatch 전 정확한 skill admission |
