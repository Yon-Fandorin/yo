...
| [`model_service/connection_repository.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/model_service/connection_repository.rs), [`model_service/selection.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/model_service/selection.rs), [`model_service/startup.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/model_service/startup.rs) | complete-binding identity 밖의 compatible absent-enabled/exact-false activation 저장, exact-state activation CAS와 preference 제거, disabled 행 표시, 새 startup·direct selection·picker acceptance·replacement·reservation에 대한 단일 typed `ModelBindingDisabled` admission failure | CLI 문구, Provider entitlement, credential mutation 또는 admit된 Turn 취소 |
...
| [`backends/delegated-grok`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/backends/delegated-grok/src/lib.rs) | `grok agent stdio` 시작, ACP v1 초기화, cached-login 인증, Session 없는 정확한 subscription-tier account snapshot, 제한된 request·Session 연결, permission mapping, event 변환, `session/load` continuation, foundation transport 위의 결정론적 정리 | provider 중립 runtime 의미, 다른 delegated host, 직접 xAI Provider 접근 또는 설치된 host가 보고하지 않은 quota window 추론 |
...
| [`runner/model.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/model.rs) | 저장 activation을 공용 selection panel에 Projection하고 disabled model을 `disabled by operator`와 함께 보이게 유지하면서 row acceptance를 차단 | Durable activation mutation, startup admission 또는 범용 overlay interaction |
