...
| [`backends/delegated-grok`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/backends/delegated-grok/src/lib.rs) | `grok agent stdio` 시작, ACP v1 초기화, cached-login 인증, 제한된 request·Session 연결, permission mapping, event 변환, `session/load` continuation, foundation transport 위의 결정론적 정리 | provider 중립 runtime 의미, 다른 delegated host 또는 직접 xAI Provider 접근 |
