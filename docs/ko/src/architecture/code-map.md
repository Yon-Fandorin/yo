...
| [`model_binding_admission.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/model_binding_admission.rs), [`model_profile_admission.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/model_profile_admission.rs) | 필수 secret-free `ModelBindingAdmission` port와 중립 reasoning/tool/replay 결과. generic OpenAI-dialect admission은 Kimi를 명시적으로 거절한다. external connection 준비와 managed 생성은 기본 구현이나 fallback 없이 주입된 validator를 사용 | Kimi compatibility policy, authored profile 해석, Provider wire 직렬화, connector transport 또는 durable binding identity |
