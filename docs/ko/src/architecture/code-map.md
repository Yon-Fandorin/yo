...
| [`prompt`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/prompt/mod.rs) | editor 내용과 cursor가 보이는 상태를 측정하고 그리며, 유효한 `@` token scan, stale provider update 거절, 선택 span 치환과 typed identity 보존 | 편집 의미는 `input/editor`, 탐색은 실행 provider, structured admission은 `yo-core` |
