...
| 경계 | 소유하는 책임 | 소유하지 않는 책임 |
|---|---|---|
| [`src/main.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/main.rs), [`src/lib.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/lib.rs), [`src/application.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/application.rs), [`src/application/runtime.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/application/runtime.rs), [`src/application/runtime/session.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/application/runtime/session.rs), [`src/application/runtime/startup.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/application/runtime/startup.rs), [`src/application/runtime/live.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/application/runtime/live.rs), [`src/application/runtime/frontend.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/application/runtime/frontend.rs), [`src/application/runtime/print.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/application/runtime/print.rs), [`src/application/output.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/application/output.rs), [`src/application/codex_diagnostics.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/application/codex_diagnostics.rs) | `main.rs`는 지원 플랫폼 진입점을 선택하고, Unix 조건부인 `lib.rs`는 공개 `run` facade를 제공한다. `application.rs`는 인자 해석과 최상위 dispatch를, `application/runtime.rs`는 공유 lifecycle owner를 선언하고 재-export하며, `runtime/session.rs`는 live-session 상태·step outcome·agent shutdown을, `runtime/startup.rs`는 startup snapshot·storage·resume 검증·provider startup을, `runtime/live.rs`는 terminal generation 재진입과 process cleanup을, `runtime/frontend.rs`는 TUI 구성과 terminal generation을, `runtime/print.rs`는 one-shot projection finalization을, `application/output.rs`는 Provider 중립 stdout/stderr와 `CliDiagnostic` 최종화를, `application/codex_diagnostics.rs`는 Codex warning 관찰·중복 제거·bounded publication·오류 조합을 소유한다 | 에이전트 의미나 터미널 렌더링 |
...
| [`src/command/`](https://github.com/Yon-Fandorin/yo/tree/develop/crates/yo-cli/src/command), [`src/command/account/`](https://github.com/Yon-Fandorin/yo/tree/develop/crates/yo-cli/src/command/account), [`src/command/session/`](https://github.com/Yon-Fandorin/yo/tree/develop/crates/yo-cli/src/command/session), [`src/command/usage/`](https://github.com/Yon-Fandorin/yo/tree/develop/crates/yo-cli/src/command/usage), [`src/live.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/live.rs), [`src/state/config.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/state/config.rs) | live와 one-shot print startup, `yo default`, exact 또는 grouped `yo connect`, exact 또는 interactive `yo disconnect`, `yo session`, 최상위 `yo usage`, cache 또는 명시적 refresh를 지원하는 `yo account` 문법 분리, command가 소유하는 저장 Session list/history/presentation과 typed Usage 투영, account 소유 target 해석·provider refresh·bounded cache 저장·table/detail/JSON 표시, Provider 중립 text·JSON account-capacity 표시, test 가능한 live 경계에서 `yo --resume UUID`와 현재 workspace의 `yo --continue` 선택, `yo-yaml`을 통해 command-local 일반 설정을 no-follow handle 하나로 안정적으로 capture하고 mutation 직전에 다시 확인, startup-only TUI frame rate와 Session 목록 날짜 설정, TTY 폭에 따른 열 우선순위, print 응답·저장 Chat·bounded Transcript·Request·typed Usage·account capacity와 discovery mismatch의 stdout/stderr routing | 모델 definition이나 preference, physical Session decode, semantic recovery, provider-native resume, 실행 중 설정 reload, Provider wire decode, 범용 반응형 plain-text layout |
| [`src/print.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/print.rs) | UTF-8 stdin과 prompt 조합, 변경 불가능한 Submission 정확히 하나, nonblocking Session polling, 대화형 요청의 실패-폐쇄 처리, 마지막으로 완료된 Agent message만 줄바꿈을 붙여 stdout에 투영 | Backend round, tool policy, Session 저장, usage 집계, provider protocol 또는 터미널 표시 |
| [`src/state/connection/operation.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/state/connection/operation.rs), [`src/command/connect/import.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/command/connect/import.rs), [`src/command/connect/external.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/command/connect/external.rs), [`src/command/disconnect/execution.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/command/disconnect/execution.rs), [`src/connection/input.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/connection/input.rs), [`src/command/connect/input/file.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/command/connect/input/file.rs), [`src/command/connect/picker.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/command/connect/picker.rs), [`src/connection/presentation.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/connection/presentation.rs) | 공유 recovery lane 하나에서 `yo default`, Local Codex·Local Grok connect, exact 저장 external-model connect, bounded grouped `yo connect --from` import, 대화형 OpenRouter·Kimi account-model discovery, 로컬 QwenCloud plan catalog 선택, target 하나의 저장 disconnect 조율, manual/stored 합성 계층 없는 whole-pair atomic replacement, unavailable account model도 정확한 disabled 이유와 함께 보여 주고 정리 소유자가 하나인 bounded controlling-TTY 검색·스크롤 picker, Kimi local-private-replay disclosure와 exact 보조 상세를 포함하는 connect·import·disconnect preview, controlling-TTY target·확인·no-echo credential 입력, non-interactive import의 exact absolute definition·credential-file channel, 최종 config guard, 최초 성공 winner 보존, 저장 preference의 startup capture | 자유 형식 Provider onboarding, 추가 non-interactive secret channel, 범용 CLI widget framework 또는 repository의 physical storage |
| [`src/command/model/mod.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/command/model/mod.rs), [`src/state/connection/operation.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/state/connection/operation.rs), [`src/execution/model/startup.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/execution/model/startup.rs) | 닫힌 `yo model enable TARGET`·`yo model disable TARGET` 문법, Provider·credential 접근 없이 복구된 operation lane에서 수행하는 activation lookup과 idempotent public CAS, override가 없고 durable complete binding이 그대로인 exact native-resume 예외 | Durable activation encoding, catalog availability, credential 소유권 또는 이미 admit된 작업 중단 |
| [`src/execution/tools`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/execution/tools.rs) | startup에서 exact basic·legacy·empty frozen registry 복원, credential-aware semantic admission, descriptor에 고정된 no-follow workspace path, 파일별 결과를 내는 bounded batch read, exact-text edit 계획, host instance 하나의 mutation lock 아래 atomic complete-file 공개와 정리, approval-bound command process | tool-call 순서, semantic Activity, provider transport, hostile same-UID namespace 보호 또는 Session Journal 저장 |
| [`src/execution/model/native.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/execution/model/native.rs) | 이미 도출된 exact Connector identity와 API dialect를 concrete Connector factory 하나에 매핑하고 그 Connector를 Yo-managed loop에 주입하며 Provider probing이나 fallback 없이 host 소유 tokenizer·tool·request observation service를 조립 | Connector request grammar, model-loop 의미 또는 binding 도출 |
| [`src/state/storage.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/state/storage.rs) | 사용자별 플랫폼 상태 루트와 별도 override 가능한 Session repository 루트 선택, local writer·생성하지 않는 reader·Host-identity-only 경로를 분리해 조합. live writer startup과 local delegated-host 검증은 안정적인 Host identity 하나를 공유하고 read-only command는 기존 identity와 repository만 관찰 | Host identity의 의미나 physical Session record 의미 |
...
| [`connectors/kimi`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/connectors/kimi/src/lib.rs) | 중립 Connector port의 concrete Kimi Chat Completions 구현. Platform/Code matrix를 독립적으로 재검증하고 Kimi request admission·직렬화, 정확한 cache-read usage availability를 포함한 stream 문법, provider-private assistant codec·visible projection, exact replay-size 계산을 전적으로 소유하며 `yo-cli`가 exact Kimi identity와 dialect에만 선택한다 | catalog discovery와 secret-free profile compatibility, durable opaque-envelope 저장, 공통 transport byte 또는 managed semantic Activity |
...
| [`meter`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/meter/mod.rs) | 재사용 가능한 terminal-safe percentage meter. vertical-level·horizontal-bar·vertical-bar shape, profile 또는 custom glyph family, 검증된 layout template을 제공한다 | 호출 측 presentation 정책, semantic threshold 색상, 표의 컬럼 의미 또는 terminal ownership |
