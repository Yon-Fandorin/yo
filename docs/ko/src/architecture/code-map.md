...
| [`backend/native`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/backend/native/mod.rs) | Yo가 관리하는 Responses loop, semantic model/tool Activity, 직렬 validation·admission·approval·execution 순서, request별 context admission, 제한된 model round, 응답별 정확한 binding·usage 귀속, cancellation 정리, 제한된 provider 중립 replay delta, 소진 시 completed non-resumable 처리와 binding latch | startup model 선택, tokenizer 구현, semantic-admission policy, 구체적인 local tool 구현 |
