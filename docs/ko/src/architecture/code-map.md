...
| [`model_connector`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/model_connector/mod.rs) | optional request cap을 생략할 수 있는 provider 중립 `openai-responses`·`openai-chat-completions` request 직렬화, admitted hard maximum 안의 정확한 양수 request cap을 요구하고 제한하는 제품 범위가 닫힌 `kimi-chat-completions` Platform·Code K3/K2.7/K2.6 envelope, Code에서만 직렬화하는 redacted opaque cache-affinity request hint, request-local tool 노출, exact Kimi private-assistant 재구성, 같은 origin으로 제한한 bounded redirect, bearer 인증 HTTPS, 취소 가능한 request worker, dialect별 bounded SSE framing·correlation·finish/terminal 상태·usage·Kimi private-assistant emission, warning-only connection observation을 위한 body 독립 closed request-failure 분류 | semantic Activity와 tool loop는 Yo-managed backend |
