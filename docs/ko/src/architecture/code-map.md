...
- 서비스별 catalog·discovery·account capacity는 `yo-provider-*`, wire grammar와 private replay는 `yo-connector-*`에 있다.
...
| [`yo-test-support`](https://github.com/Yon-Fandorin/yo/blob/develop/shared/yo-test-support/src/lib.rs) | Dev-dependency로만 사용하는 bounded local TLS fixture와 child 진단 | Production network 동작 또는 Provider grammar |
...
| 경계 | 소유하는 책임 | 소유하지 않는 책임 |
|---|---|---|
| [`src/main.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/main.rs), [`src/lib.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/lib.rs), [`src/application.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/application.rs), [`src/application/`](https://github.com/Yon-Fandorin/yo/tree/develop/crates/yo-cli/src/application) | 공개 진입 dispatch, live·one-shot lifecycle 조율, 구체적인 TUI `AgentConnection` adapter, resume·continue 선택, stdout/stderr 최종화, Codex warning 공개 | command 문법, durable state encoding, backend 의미 또는 terminal 렌더링 |
| [`src/command.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/command.rs), [`src/command/`](https://github.com/Yon-Fandorin/yo/tree/develop/crates/yo-cli/src/command) | 최상위 문법과 account, connect, default, disconnect, model activation, live, print, Session 조회, Usage 출력의 vertical slice | 공유 process lifecycle, storage 구현 또는 범용 interaction primitive |
| [`src/execution.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/execution.rs), [`src/execution/`](https://github.com/Yon-Fandorin/yo/tree/develop/crates/yo-cli/src/execution) | host 소유 model 선택·backend 조립, delegated host 검증, managed local tool, Unix job control·외부 편집기 프로세스 소유·termination 조율 | command 정책, durable connection mutation 또는 사용자 대상 문구 |
| [`src/interaction.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/interaction.rs), [`src/interaction/`](https://github.com/Yon-Fandorin/yo/tree/develop/crates/yo-cli/src/interaction) | 공유 CLI style·diagnostic, controlling-TTY prompt, connection 확인·결과 표시 | command별 출력 구조, repository mutation 또는 TUI 렌더링 |
| [`src/state.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/state.rs), [`src/state/`](https://github.com/Yon-Fandorin/yo/tree/develop/crates/yo-cli/src/state) | 일반 configuration capture, 명시적인 상대 `YO_CONFIG`의 일회성 절대 경로 고정, connection operation·startup state, platform state root 선택, Session storage 조합, stable Host identity 접근 | physical Session 의미, Provider protocol 동작 또는 process 실행 정책 |

프로세스 시작이나 종료가 실패하면 `main.rs`, `lib.rs`, `application/runtime/{startup,live,frontend,print}.rs`에서 오류 문맥에 표시된
...
| [`host`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/host.rs) | 불투명한 random UUIDv4 `WorkspaceHostId`, 원자적으로 만들고 권한을 제한한 local 사용자별 identity 파일, 생성 Host가 만든 lossless canonical workspace path 값 | Host identity가 일치할 때의 workspace 비교와 remote Host transport |
...
| [`skill_reference`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/skill_reference.rs) | frontend에 독립적인 skill identity, 실행 환경 provenance, catalog generation과 entry revision selector, availability, revision-bound 검색 메시지 | TUI 표시와 제출 시점의 정확한 재검증. 구체적인 catalog adapter는 `backend` 아래에 남는다 |
| [`input`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/input.rs) | 변경 불가능한 제출 text, 화면의 정확한 byte span에 묶인 순서 있는 typed reference occurrence, 안전한 reference token의 canonical Projection, UUIDv4 submission 연결, 제출 전체의 최종 outcome | queue와 worker 수락은 `agent_session`. 구체적인 reference admission은 다음 경계에 남는다 |
| [`interview`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/interview.rs), [`interview/repository.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/interview/repository.rs), [`interview/working_copy.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/interview/working_copy.rs), [`interview/recovery.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/interview/recovery.rs) | 완전한 인터뷰 요청 캡처, 문맥 초안 `yo.interview-draft/v1`, generation-CAS 저장, 비밀 값 배제, 이전 암호화 복구 항목의 정리 | 실행 중 hidden editor 동작은 `yo-tui/runner/state/interview.rs`, platform state·configuration root 선택은 `yo-cli/state/storage.rs`, 인증된 destination 근거는 backend adapter |
| [`model_service`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/model_service.rs) | 안정적인 Provider·Account·Model identity, 명시적인 API dialect 선택과 정확히 하나의 built-in connector 파생, 정규화한 HTTPS endpoint, Provider·Account base profile과 whole-field model override를 완전한 typed profile 하나로 해석하는 경계. 여기에는 presence를 보존하는 optional known output maximum, unknown일 때의 생략, whole-field null 거절이 포함된다. 닫힌 source tag와 built-in identity 검증을 갖는 중립 persisted catalog descriptor, startup과 native resume이 공유하는 complete-binding 값 하나와 닫힌 durable decoder, `yo-yaml`로 decode하는 현재 pre-version `connections.yaml`·`credentials.yaml`·connection-operation shape, Provider·Account 범위 catalog·context-profile·credential resolution, 주입하는 tokenizer-counting port, 원문을 감춘 resolved credential, 안전하고 크기가 제한된 local `credentials.yaml` private-revision exact-pair CAS, account·complete model·catalog seed·preference·warning-only model별 failure observation의 유일한 owner인 mode `0600`의 bounded `connections.yaml`, 복구한 operation lane에서 exact binding과 private credential revision을 재검증한 뒤 수행하는 조건부 observation CAS, whole-pair replacement와 exact-model mutation, 닫힌 durable phase, connect 시 모델 요청 없이 수행하는 구조적 binding admission, public-first disconnect 실행, 순수 exact-state recovery table을 가진 secret-free bounded `connection-operation.yaml` intent repository, operation lock 하나를 유지하며 journal·credential·public phase를 commit하는 same-directory local executor | 범용 Provider discovery, connector wire 변환, command 수준 target·확인 표시 또는 설정 경로 선택은 process host |
| [`model_service/connection_repository.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/model_service/connection_repository.rs), [`model_service/selection.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/model_service/selection.rs), [`model_service/startup.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/model_service/startup.rs) | complete-binding identity 밖의 compatible absent-enabled/exact-false activation 저장, exact-state activation CAS와 preference 제거, disabled 행 표시, 서로 분리된 managed target과 revision-bound delegated-host picker target, 안정적인 fingerprint host AccountId, current section 우선 정렬과 inline ` (current)`를 포함한 account-section Projection, same-account active-host availability와 disabled sibling Projection, 새 startup·direct selection·picker acceptance·replacement·reservation에 대한 단일 typed `ModelBindingDisabled` admission failure | CLI 문구, host protocol decoding, credential mutation 또는 cross-runtime semantic handoff |
| [`providers/kimi/catalog.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/providers/kimi/src/catalog.rs), [`providers/kimi/catalog`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/providers/kimi/src/catalog) | 분리된 Platform AI·Code Membership catalog profile과 endpoint, bounded 인증 Kimi `/models` snapshot 하나, 별도의 exact-once bounded Code Membership `/me` account-level snapshot과 `/usages` account-capacity snapshot, 첫 valid duplicate 처리, 제품 범위의 검토된 exact-model overlay, 완전한 selectable binding, 결정론적 순서, 유지한 unavailable 행마다 표시하는 typed reason | Kimi request/stream 변환, private Session replay, Platform account capacity 또는 CLI consent·표시 |
| [`providers/openrouter`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/providers/openrouter/src/lib.rs), [`providers/qwencloud`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/providers/qwencloud/src/lib.rs) | Provider별 catalog 재구성·검증, OpenRouter bounded discovery·normalization·availability, QwenCloud catalog 및 browser-session account-capacity HTTP·해석 | CLI 입력·재시도·credential CAS, core 저장 schema, connector wire grammar |
| [`model_binding_admission.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/model_binding_admission.rs), [`model_profile_admission.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/model_profile_admission.rs) | 필수 secret-free `ModelBindingAdmission` port와 중립 reasoning/tool/replay 결과와 typed admitted Chat image wire policy. generic OpenAI-dialect admission은 Kimi를 명시적으로 거절한다. external connection 준비와 managed 생성은 기본 구현이나 fallback 없이 주입된 validator를 사용 | Kimi compatibility policy, authored profile 해석, Provider wire 직렬화, connector transport 또는 durable binding identity |
| [`model_connector`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/model_connector.rs) | 공개 provider 중립 Connector·stream port와 공유 request·observation·취소·failure·limit·opaque provider-private envelope·visible projection 어휘. 구체적인 Provider request, stream, private payload 문법은 core에 두지 않는다 | provider wire grammar 또는 Yo-managed backend의 semantic Activity·tool loop |
...
| [`connectors/openai-chat-completions`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/connectors/openai-chat-completions/src/lib.rs) | 중립 Connector port의 concrete OpenAI-compatible Chat Completions 구현으로, 순서 있는 message·tool replay 직렬화와 크기가 제한된 finish–usage–`[DONE]` stream grammar를 소유. `yo-cli`가 exact Chat identity와 dialect에만 이 구현을 선택한다 | Kimi private replay, model 선택, 공통 transport byte 또는 managed semantic Activity |
| [`connectors/kimi`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/connectors/kimi/src/lib.rs) | 중립 Connector port의 concrete Kimi Chat Completions 구현. 하나의 canonical Platform/Code matrix가 공개 secret-free binding admission과 직접 connector 생성을 함께 검증한다. Kimi request 직렬화, 정확한 cache-read usage availability를 포함한 stream 문법, provider-private assistant codec·visible projection, exact replay-size 계산을 소유하며 `yo-cli`가 exact Kimi identity와 dialect에만 선택한다 | catalog discovery, durable opaque-envelope 저장, 공통 transport byte 또는 managed semantic Activity |
| [`tool`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/tool.rs) | 안정적인 tool identity, 요청마다 고정한 registry, 닫힌 `yo.tool-schema/v1` dialect에 대한 크기 제한 인자 검증, argument와 output projection을 위한 주입형 semantic admission, 정규화한 approval binding, typed effect, 주입하는 단일 시도 execution-host port | 구체적인 운영체제 effect나 provider-hosted tool |
| [`engine`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/engine.rs) | 결정론적인 Session, Turn, Activity, request 상태 전이 | 전이가 provider 경계도 지난다면 `runtime` |
| [`journal`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/journal.rs), [`request_trace.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/request_trace.rs) | commit된 command, semantic event, redacted context-policy/checkpoint observation, 내부 backend correlation fact를 하나의 순서로 보존하는 live Projection, correlation 전용 record를 제외하는 sequence 기반의 제한된 Transcript 읽기, correlation 좌표를 보존하는 payload-free Request trace 읽기, live와 stored가 공유하는 Request Projection model, 동기식 durable publication, typed gap 상태, revision을 인식하는 크기 제한 `MessageSegment` 구성, recovery 검증. 실패한 semantic outcome은 message와 함께 명시적인 nullable code를 저장한다. 비공개 codec은 semantic `JournalSequence`와 physical replay 좌표를 분리하고 제한된 replay chain을 증분 검증하며 backend exchange, binding epoch, deterministic internal successor identity, accepted request, resumable outcome, Continuation Anchor, 단조 context policy, 정확한 source group, 원자적 context checkpoint를 하나의 correlation graph로 검증한다 | capture 지점은 `runtime`, physical durability는 `session_repository` |
| [`session_repository`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/session_repository.rs) | 저장 형식에 독립적인 append·replay·저장 Session 탐색/읽기 포트, snapshot 복구 gate, typed storage pressure, 첫 Session-single-writer versioned-JSONL 로컬 구현. 여러 process가 안정적인 root 하나를 함께 열고 서로 다른 Session을 동시에 쓸 수 있다. Writer-capable instance마다 legacy compatibility shared guard를 유지하고, Session 하나를 load하기 전에 exclusive lease를 얻으며, 최종 용량 확인과 physical append 동안에만 짧은 root coordinator를 얻는다. 현재 physical `v1` envelope는 checksummed discovery summary를 가진다. `LocalSessionReader`는 writer lease나 변경 없이 기존 저장소를 열고 descriptor로 그 tree를 고정하며 nonblocking no-follow open으로 symlink와 일반 파일이 아닌 entry를 거부한다. Session마다 검증한 tail envelope 하나로 목록을 만들며, 존재 여부를 포함해 한 시점으로 고정된 history를 읽는다. `read_stored_session`은 파일 없음과 파일은 있지만 complete envelope가 없음을 구분하고, physical envelope와 semantic recovery를 검증하고 저장 전용 message segment를 semantic snapshot으로 합치며, message-recovery interruption, physical sequence를 포함한 최초 typed discovery mismatch, `v1`만으로는 종료 뒤 durability continuity를 관찰할 수 없다는 사실을 보존한다. 같은 검증된 recovery는 durable backend correlation fact 전체에서 frontend 독립적인 payload-free Request trace를 Journal 순서로 도출하며, physical envelope나 Request Audit payload는 노출하지 않는다. semantic recovery가 correlation chain을 증명한 뒤에만 binding epoch와 Continuation Anchor가 discovery에 들어간다. 저장 history를 읽을 때도 각 physical commit 지점에서 같은 상태를 다시 도출하여 summary가 빠졌거나 모순되면 거부한다. `read_stored_session_continuation`은 변경 없이 후보를 검증하고, `recover_stored_session_continuation`은 Session writer lease 안에서 같은 recovery를 반복한 뒤 descriptor, 최신 durable Anchor와 binding evidence, 복원할 semantic prefix와 frontend observation, 다음 Turn identity, 이미 승인한 Submission identity를 typed unit 하나로 반환한다. local `reader`와 `file` 모듈은 관찰과 변경 책임을 나눈다. `JournalRepository`는 candidate를 durable semantic prefix와 검증하고 semantic commit 하나를 physical append 하나와 조합 | provider-native resume, remote storage나 transport, Request Audit persistence, database나 compression 대안 |
| [`runtime`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/runtime.rs) | backend 수락, semantic commit, Journal capture 순서, binding·context epoch와 SubmissionId 기반 command root 및 별도의 writer-assigned 내부 request identity 소유, provider 중립적인 binding/request/outcome evidence·단조 증가하는 completed active suffix·sequence-free checkpoint proposal 검증, post-tool replay를 닫힌 Journal Activity 경계에 결속, 첫 request 전 policy와 successor request 수락 전 checkpoint commit, 완전한 continuation chain과 exact-replay 또는 backend-native model-rebind 전환의 원자적 공개, codec으로 검증한 durable prefix에서 결정론적 Engine과 exact replay-group 상태 복원, 재개한 backend identity 검증 뒤 full recovery snapshot 공개, 실패 시 활성 작업 종료 | provider port는 `backend/contract.rs` |
| [`agent_session`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/agent_session.rs), [`startup`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/agent_session/startup.rs), [`fork`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/agent_session/fork.rs), [`replacement`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/agent_session/replacement.rs), [`lifecycle`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/agent_session/lifecycle.rs), [`observation`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/agent_session/observation.rs), [`worker`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/agent_session/worker.rs) | frontend를 막지 않는 접근, 크기가 제한된 command lane, backpressure 동안 유지되는 submission identity, checkpoint activation까지 뒤따른 prompt와 binding replacement를 막는 admission-time idle-compaction reservation, worker가 확정하는 submission·nonterminal control outcome, reservation 해제를 포함하는 용량 1의 Journal 변경 알림, Changed-before-terminal 순서를 지키고 shutdown이 poll되지 않은 failure를 회수하게 하는 별도 terminal payload 하나, 시작 취소, 종료 조율, 검증된 continuation에서 다음 Turn과 승인된 Submission identity를 복원하는 startup hydration | worker가 소유하는 의미 처리는 `runtime` |
| [`backend/contract.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/backend/contract.rs), [`backend/evidence.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/backend/evidence.rs) | `yo-backend::BackendAdapter`를 Yo의 `AgentCommand`, `BackendEvent`, durable resume target, advertise된 native-model-rebind capability, optional source anchor로 고정한 `AgentBackend` specialization. Session·Journal 좌표와 exact replay-profile·schema 해석은 core에 유지한다 | generic lifecycle·evidence mechanics, concrete adapter 또는 Provider wire grammar |
| [`backends/delegated-codex`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/backends/delegated-codex/src/lib.rs) | `codex app-server` 시작과 protocol 분류, provider ID 연결, core event 변환, account를 포함하는 effective-model/thread continuation evidence, ephemeral이 아닌 저장 thread, 최신 durable locator에 대한 검증된 `thread/resume` 한 번, native rebind를 위한 검증된 same-account exact-model `thread/fork` 한 번, Session 없는 인증형 `account/read`와 bounded paginated visible `model/list` inventory, foundation transport 위에서 worker가 소유하는 `skills/list` metadata catalog, 검토한 wire의 요청 결속형 비밀 전달과 직접 도구 결과 보호 | provider 중립 runtime 의미, 다른 delegated host, structured dispatch 전 정확한 skill admission, host/model의 보관 동작 또는 atomic replacement policy |
| [`backends/delegated-grok`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/backends/delegated-grok/src/lib.rs) | `grok agent stdio` 시작, ACP v1 초기화, cached-login 인증, Session 없는 정확한 account와 advertised `modelState` inventory snapshot, 제한된 request·Session 연결, permission mapping, event 변환, `session/load` continuation, foundation transport 위의 결정론적 정리, bounded writer 확인을 포함한 HTTP-MCP 요청 결속형 비밀 전달, 나뉘어 오는 도구 identity가 확인될 때까지의 결과 게시 보류, 직접 비밀 도구 결과 보호, state-preserving model-rebind capability를 advertise하지 않는다는 명시적 경계 | provider 중립 runtime 의미, 다른 delegated host, 직접 xAI Provider 접근, host/model의 보관 동작, 설치된 host가 보고하지 않은 quota window 추론 또는 semantic handoff policy |
| [`backends/managed`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/backends/managed/src/lib.rs), [`backends/managed/src/backend.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/backends/managed/src/backend.rs), [`backends/managed/src/backend/context.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/backends/managed/src/backend/context.rs) | connector-neutral observation을 사용하는 provider 중립 Yo-managed model loop, semantic model/tool Activity, 직렬 validation·admission·approval·execution 순서, dispatch 전 누적 retained+current replay admission, 제한되고 엄격히 감소하는 request-cap 선택과 exact final-payload 재계산, 제한된 model round, visible refusal replay, completed visible projection과 exact replay-profile schema가 맞은 뒤에만 받는 opaque provider-private replay, Provider 분기 없이 붙이는 opaque Session-stable cache-affinity hint 하나, response·round마다 exact binding·token·닫힌 cache-read availability 귀속을 담는 durable `yo.model-usage-receipt/v1` 하나, 85/90 exact context-pressure admission, tools-disabled fixed-shape portable summary 한 번, completed post-tool active-suffix 공개, summary 뒤 재계산, checkpoint-before-dispatch handshake, idle `/compact`, resume-safe context group, cancellation 정리, 제한된 replay delta, pre-final context exhaustion의 typed failure와 binding latch, anchor 없는 final-delta 전용 completed non-resumable 예외 | startup model 선택, tokenizer 구현, provider-private payload 해석, semantic-admission policy 또는 구체적인 local tool 구현 |
...
지점이다. Codex와 Grok wire value는 각각의 `backends/delegated-*` crate 아래에 있고 Connector 중립 loop는
...
직접 지정의 읽기 전용 Chat·Transcript·Request 출력에 이 포트를 사용한다. Transcript만
최신 record 선택과 payload 노출 제한을 허용한다. 별도 `yo usage` 명령은 같은 복구 history로
typed 영수증 보고서를 만들며, 두 명령 모두 어떤 항목도 실행 가능하게 만들지 않는다.
...
delegated-host native resume은 연결되어 있다. resume에는 완결된 durable Continuation Anchor
...
| 모듈 | 소유하는 책임 | 다음 탐색 지점 |
|---|---|---|
| [`runner`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner.rs) | 실행 중인 session의 공개 facade, 터미널을 단독 소유하는 loop, 모든 live source의 필수 readiness, 무기한 idle 대기, 설정 가능한 120/60fps frame 합치기, 보존되는 Inline Chat publication cursor, 마지막 정리 결과 보고, 터미널에 독립적인 저장 Chat·Transcript·Request Projection | 공통 상태·data facade는 `runner/state.rs`, input·follow-up은 `runner/state/input.rs`, slash command·Session 전환은 `runner/state/commands.rs`, interview working copy는 `runner/state/interview.rs`, Journal·Chat 관찰은 `runner/state/observation.rs`, request correlation은 `runner/state/requests.rs`, opt-in live attention identity와 dedupe는 `runner/attention.rs`; persistent 행 준비와 compact live size는 `runner/publication.rs`, frame-rate 정책은 `runner/frame.rs`, 저장 출력은 `runner/archival.rs`, 실행 중 조율·flush 후 geometry 관찰·owner-thread BEL 전송·보이는 motion scheduling은 `runner/unix.rs`·`runner/unix/presenter.rs` |
| [`runner/state.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/state.rs) | `StateEffect`·`StateError`, `TuiState`·`PendingRequest` data, 생성·accessor, 공통 frame commit과 cross-cutting 조율을 제공하는 얇은 `TuiState` facade | 세부 상태 동작은 `runner/state/input.rs`, `runner/state/commands.rs`, `runner/state/interview.rs`, `runner/state/observation.rs`, `runner/state/requests.rs`; image·frame·preview 소유권은 기존 `runner/state/image.rs`, `runner/state/presentation.rs`, `runner/state/preview.rs`에 유지 |
| [`runner/session.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/session.rs), [`runner/session/metadata.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/session/metadata.rs), [`runner/session/construction.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/session/construction.rs), [`runner/session/continuation.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/session/continuation.rs), [`runner/session/bridges.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/session/bridges.rs), [`runner/session/generation.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/session/generation.rs) | `TuiSession` facade는 터미널과 독립적인 상태를 유지한다. `metadata.rs`는 resume entry, host status/document metadata와 크기가 제한된 표시 형식을 소유하고, `construction.rs`는 생성자와 session 전용 host/rendering 설정을 소유한다. `continuation.rs`는 inherited history, resume/session-tree/fork picker, new-session/model 전환을 소유하고, `bridges.rs`는 interview·image-preparation·status-line·appearance bridge를 소유한다. `generation.rs`는 runner가 사용하는 session parts와 publication-recovery evidence를 소유한다 | 공통으로 유지되는 상태와 frame commit은 `runner/state.rs`, 터미널 세대 자원은 `runner/unix.rs`, semantic Session과 continuation 의미는 `yo-core` |
| [`runner/state/input.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/state/input.rs), [`runner/state/commands.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/state/commands.rs), [`runner/state/interview.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/state/interview.rs), [`runner/state/observation.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/state/observation.rs), [`runner/state/requests.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/state/requests.rs) | `input.rs`는 입력 우선순위·편집기 제출·후속 입력 예약과 요청에 묶인 비밀 입력, `commands.rs`는 slash command와 Session 전환, `interview.rs`는 문맥 초안과 이전 vault 정리, `observation.rs`는 Journal·내구성·참조·Chat 관찰, `requests.rs`는 요청 표시와 질문·승인 응답 연결을 소유한다 | 공통 data와 frame/publication commit은 `runner/state.rs`, Session·interview·request 의미는 `yo-core`로 이동 |
| [`runner/state/history.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/state/history.rs) | 현재 Session의 승인 prompt를 bounded recall하고, 확정 일반 turn에서 resume seed를 만들며 검색 panel과 초안을 복원 | 접수 결과와 입력 우선순위는 `runner/state/input.rs`, bounded resume carrier는 `runner/session/continuation.rs`와 `yo-cli/runtime/startup`, 참조·이미지 복원은 `prompt/assist.rs` |
| [`runner/state/find.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/state/find.rs) | 확정된 일반 Chat 메시지를 범위 안에서 읽기 전용으로 검색하고, 결과 선택 panel을 소유하며, 미전송 초안을 보존 | 입력 우선순위는 `runner/state/input.rs`, 의미상 item 이동은 `runner/view.rs`와 `transcript/viewport.rs` |
| [`runner/session/external_editor.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/session/external_editor.rs), [`runner/state/external_editor.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/state/external_editor.rs) | terminal에 종속되지 않은 외부 편집기 snapshot, 일반 초안 자격, 오래된 결과 거절, 가져올 때 typed 참조·이미지 보존 | 복원된 terminal 인계는 `runner/unix.rs`, 비공개 파일·자식 프로세스는 `yo-cli/execution/process/external_editor.rs` |
| [`runner/chat.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/chat.rs) | 커밋된 Chat 투영과 `/copy`용 마지막 완료 답변 원문 선택 | 명령 접수는 `runner/state/commands.rs`, 제한된 터미널 쓰기는 `terminal/clipboard.rs` |
...
| [`runner/view.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/view.rs) | Chat·Transcript·Request 선택, 상단 헤더가 없는 편집 가능한 Chat 화면, 읽기 전용 mode 헤더, 전체 Transcript Projection, 선택적인 정확한 context 강조를 포함한 Session 전체 payload-free Request trace, mode별 context·viewport 상태 | Journal·Chat 관찰은 `runner/state/observation.rs`, editor dispatch는 `runner/state/input.rs`, request correlation은 `runner/state/requests.rs`, 공통 layout·scroll은 `transcript` |
| [`prompt`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/prompt.rs) | editor 내용과 cursor가 보이는 상태를 측정하고 그리며, 유효한 `@`·`$` token scan, 대체 query가 pending인 동안 마지막 usable panel 보존, stale provider update 거절, 선택 span 치환과 typed identity 보존, 보고된 scope에 따른 cached skill 후보 filtering | 편집 의미는 `input/editor`, 탐색은 실행 provider, freshness-gated 표시는 `overlay`, structured admission은 `yo-core` |
| [`overlay`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/overlay.rs) | 검증된 선택 panel snapshot, 항목 availability와 독립적인 semantic snapshot freshness, typed static/activity title status, enabled 항목 navigation·fitting, 선택적인 왼쪽 하단 filter 표시, 원자적인 `Surface` paint, refresh된 selection을 일치하는 frame commit까지 fence하는 token·revision presentation receipt를 가진 단일 prompt-overlay slot | provider는 query·후보 filtering·preview와 accept된 제품 effect를 유지하고, request-overlay routing은 `runner/state/requests.rs`, 공통 frame receipt commit은 `runner/state.rs`, 아래에 고정된 목적지는 `shell`이 소유한다 |
| [`shell`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/shell.rs) | overflow를 typed 오류로 보고하며 Inline live 영역의 자연 높이를 측정하고 작업·prompt·metric·도움말 stack을 배분하며, 상태에 맞는 도움말을 원자적 우선순위 segment로 폭에 맞추고 pinned activity frame과 고정 문구 style sheen을 그린 뒤 완성된 frame의 cursor와 가장 짧은 보이는 motion demand를 보고 | 작업 행은 `shell/chrome.rs`, footer는 `shell/chrome/help.rs`, 표기는 `input/key_notation.rs`, cell 쓰기는 `surface`, host가 실제로 아는 status metadata는 `runner/session/metadata.rs`, status-line update는 `runner/session/bridges.rs` |
| [`surface`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/surface.rs), [`text`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/text.rs) | adapter에 독립적인 cell 상태, Unicode grapheme과 너비, 경계가 있는 view, diff span, 터미널에 독립적인 text flow | Projection은 `terminal` 또는 `html` |
| [`terminal`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/terminal.rs) | typed terminal operation과 ANSI encoding | 표시 정책은 `terminal/mode`, Unix effect는 `terminal/backend` |
| [`terminal/clipboard.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/terminal/clipboard.rs) | 제한된 OSC 52 인코딩과 unbuffered 클립보드 요청 쓰기 | 출력 generation과 사용자에게 보이는 전송 결과는 `runner/unix.rs` |
| [`terminal/mode`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/terminal/mode.rs), [`terminal/backend`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/terminal/backend.rs) | 공유 transactional restoration, Inline·Fullscreen presenter, cursor 범위와 실제 scroll 증거를 가진 Inline typed-operation effect ledger, bounded write recovery, panic routing, crate-private direct unbuffered Unix 출력 경계 | operation/effect 순서와 exact correction은 `terminal/mode/inline/transaction.rs`, 정확한 downstream write와 flush 후 event는 `terminal/backend/unix`, 프로세스 signal 정책이 바뀔 때만 `yo-cli/execution/process` |
| [`html`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/html.rs) | 완성된 `Surface` 상태를 결정론적으로 브라우저에 Projection | 터미널과 브라우저 출력이 다르면 `surface` |
...
Projection 상태는 이 key 선택을 소유하지 않는다. Usage는 의도적으로 live F4 view가
아니며 완료된 durable 보고서는 `yo usage SESSION_ID`로 확인한다. Transcript는 같은 읽기
전용 Journal 경로에서 받은 모든 committed command, event, redacted context policy/checkpoint
observation을 그리고, Chat은 commit된 checkpoint를 간결한 operator notice 하나로 바꾼다.
...
[`tools/xtask/src/lib.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/tools/xtask/src/lib.rs)는
crate module registration과 re-export를 담당하는 facade다. workflow module을
선언하고 `xtask::run`을 re-export한다. [`tools/xtask/src/cli.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/tools/xtask/src/cli.rs)는
top-level CLI dispatch, argument routing, usage를 소유하며, 그 아래
`cli/check.rs`, `cli/docs.rs`, `cli/slice.rs`, `cli/slice_contract.rs`가
command-family parsing과 adapter를 소유한다. 주변 xtask module은 `yo` 제품이
아니라 이 저장소를 관리하는 구조화된 검사를 소유한다. 변경 경로와 commit
trailer를 분류하여 Slice 검수와 Developer Docs 영향을 확인하고, Rust test에
이해 가능한 인접 설명이 있는지도 검사한다.
