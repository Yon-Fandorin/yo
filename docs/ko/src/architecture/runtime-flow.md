...
시작해 다음 단어와 함께 복원된다. 붙여넣기·삭제·Ctrl+C 초안 지우기·텍스트가 바뀐 참조 완성은
각각 별도 단위다. 실행 취소는 원문과 커서만 복원하고 삭제된 파일·스킬·이미지 선택 권한은
되살리지 않는다. 이전 상태는 최대 64개·원문 합계 2 MiB를 메모리에 보관하며, 더 큰
초안에서 편집하면 이전 기록을 버린다. 제출과 프로그램의 초안 교체도 기록을 지운다.
비밀 입력은 별도 편집기를 사용하며 이 기록에 포함하지 않는다.

`/copy`는 답변이 있는 마지막 완료 Turn의 성공한 AgentMessage 중 비어 있지 않은 마지막 텍스트를
고른다. 비텍스트 답변 블록은 내부 JSON으로 복사하지 않으며, 그 Turn에 텍스트가 전혀 없으면
복사할 수 없다고 알린다. 화면 줄바꿈, 도구 출력, 추론, 알림, 실패 문구 대신 답변의 원래 Markdown과 개행을 사용한다.
모델 요청은 발생하지 않는다. TUI는 소유한 터미널 출력 경계로 최대 75,000바이트의 UTF-8
원문을 제한된 OSC 52 시퀀스로 보낸다. 완료 답변이 없거나 한도를 넘으면 클립보드에 쓰기
전에 로컬 안내를 표시한다. 출력 성공은 요청 전송만 뜻하며 연결된 터미널 또는 tmux의
수락을 확인하지는 못한다. SSH·tmux에서는 연결된 터미널의 OSC 52 지원과 tmux의 클립보드
쓰기 허용이 필요하고, 실제 붙여넣기가 종단 간 확인이다.
...
선택은 예약을 교체하며 현재 model을 고르면 예약을 취소한다.
...
| 단계 | 현재 소유자 | 확인할 내용 |
|---|---|---|
| 1 | [`yo-cli/src/main.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/main.rs), [`yo-cli/src/lib.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/lib.rs), [`yo-cli/src/application.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/application.rs), [`yo-cli/src/application/runtime/startup.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/application/runtime/startup.rs), [`yo-cli/src/application/runtime/startup/model.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/application/runtime/startup/model.rs), [`yo-cli/src/application/runtime/startup/prepare.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/application/runtime/startup/prepare.rs), [`yo-cli/src/application/runtime/startup/skills.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/application/runtime/startup/skills.rs), [`yo-cli/src/application/runtime/startup/failure.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/application/runtime/startup/failure.rs), [`yo-cli/src/application/runtime/live.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/application/runtime/live.rs), [`yo-cli/src/application/runtime/live/generation.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/application/runtime/live/generation.rs), [`yo-cli/src/application/runtime/live/new_session.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/application/runtime/live/new_session.rs), [`yo-cli/src/application/runtime/live/fork.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/application/runtime/live/fork.rs), [`yo-cli/src/application/runtime/live/resume.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/application/runtime/live/resume.rs), [`yo-cli/src/application/runtime/live/presentation.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/application/runtime/live/presentation.rs), [`yo-cli/src/state/connection/startup.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/state/connection/startup.rs) | `runtime/startup.rs`는 공용 startup 진입점과 snapshot type을 재공개하는 startup facade이며, `runtime/startup/prepare.rs`는 stored preference와 command option을 해석하고 Host identity와 Session storage를 열며 workspace를 canonicalize하고 선택한 backend와 skills를 준비하는 각 launch 경로의 조립을 담당한다. New와 Fork launch는 일치하는 UUIDv7 `SessionDescriptor`를 만들고 Resume은 저장된 descriptor를 복구한다. live와 print caller가 termination coordination을 설치하며 `runtime/live.rs`는 generation, transition, picker/tree/read-only presentation module을 조합해 live loop와 cleanup을 구성한다. Resume은 stored-preference를 읽지 않는다. |
...
| 3 | [`yo-core/agent_session/startup.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/agent_session/startup.rs) | `AgentSession::start_cancellable_with_repository`가 backend와 local repository를 `yo-agent-runtime`이라는 worker thread로 넘긴다. 종료 관찰을 막지 않으면서 시작 완료를 기다린다. |
| 4 | [`yo-core/agent_session/worker/lifecycle.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/agent_session/worker/lifecycle.rs) | `AgentWorker::initialize`가 descriptor-only Journal envelope를 먼저 시도한 뒤 `AgentRuntime`을 통해 `CreateSession`을 보낸다. storage pressure가 있으면 descriptor와 이후 activity를 복구 가능한 volatile prefix로 함께 유지한다. |
...
handshake 중에 종료 요청이 오면 [`agent_session/startup.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/agent_session/startup.rs)의 `AgentSession::start_inner`가 취소
...
초기화가 정확히 검토된 `0.153.4` 또는 `0.154.0` wire version을 보고하고, 같은 initialized client가
...
3. [`agent_session/admission.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/agent_session/admission.rs)는
   ...
   실행되기 전에 거절된다.
4. [`AgentWorker`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/agent_session/worker/lifecycle.rs)만
