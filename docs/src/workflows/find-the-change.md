...
| A delegated host accepted work, but the transcript does not update | its `backends/delegated-*/events.rs` → `AgentRuntime::poll_event` and Journal append → agent-session change notification → `yo-cli/agent` Transcript cursor → `TuiState::observe_record` → Chat transcript |
| Chat updates but Transcript or Request is wrong | `TuiState::observe_record` → `runner/view.rs` record formatting and exact context anchor → shared `transcript` layout; inspect `yo-core` only if the required semantic record or correlation is absent |
