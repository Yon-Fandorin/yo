...
shuts it down. Kimi makes one authenticated `GET /coding/v1/usages`; redirects
and retries are disabled and successful bodies are bounded to 1 MiB.
...
| Kimi Code account capacity | MoonshotAI Kimi Code commit `bd5e32f6835d0890fc52e19ed7eebc430b033b77`: [`managed-usage.ts`](https://github.com/MoonshotAI/kimi-code/blob/bd5e32f6835d0890fc52e19ed7eebc430b033b77/packages/oauth/src/managed-usage.ts), including the canonical endpoint, `/usages` request, weekly summary, rolling windows, and fixed-point booster balance | [`usage.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/model_service/kimi_catalog/usage.rs) keeps the product check, exact request, bounded parser, and neutral snapshot mapping beside the Kimi catalog seed. |
...
- A missing stored Kimi account or credential is a local configuration error and
  sends no request.
- Non-success status, redirect, wrong media type, malformed JSON, invalid reset
  time, zero limit, excess rows, or excess bytes fails the refresh instead of
  returning a partial healthy-looking report.
