...
Validation covers genuine-vs-lookalike provenance, all-answer/final completion correlation, stale answers and navigation, a multi-segment atomic initial capture, durable recovery, unsafe storage, generation conflicts/concurrent writers, separate reopening and UTF-8 limits. Live provider RPC replay and secret input/storage/recovery remain deferred. Corrected-build physical Mac input passed on the accepted runtime tree; see the [current direct input verification](terminal-matrix.md#current-mac-direct-input-verification).
