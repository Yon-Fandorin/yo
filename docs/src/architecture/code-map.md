...
| [`shell`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/shell/mod.rs) | Allocating the stable work/prompt/metrics/help stack, fitting state-valid help as atomic priority segments, painting the pinned activity marker, and reporting both the cursor and visible motion demand from one completed frame | `shell/chrome.rs` for the work row, `shell/chrome/help.rs` for the footer; `input/key_notation.rs` for labels; `surface` for cell writes; `runner/session.rs` for honest host-known status values |
