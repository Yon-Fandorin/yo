...
| [`model_service`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/model_service/mod.rs) | Stable Provider, Account, and Model identities; exact connector and API-protocol selection; normalized HTTPS endpoints; account-scoped catalog and context-profile resolution; an injected tokenizer-counting port; redacted resolved credentials; and the secure, bounded, startup-only local `credentials.yaml` snapshot | A connector for request translation and the process host for configuration-path selection |
| [`model_connector`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/model_connector/mod.rs) | Generic `openai-responses` request serialization, same-origin bounded redirects, bearer-authenticated HTTPS, cancellable request workers, bounded SSE framing, output-item correlation, terminal status, and token usage | A Yo-managed backend for semantic Activities and the tool loop |
...
| [`backend/native`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/backend/native/mod.rs) | The Yo-managed Responses loop, semantic model/tool Activities, serial validation-admission-approval-execution ordering, per-request context admission, bounded model rounds, exact per-response binding and usage attribution, cancellation cleanup, bounded provider-neutral replay deltas, and completed non-resumable exhaustion with binding latching | Startup model selection, tokenizer implementations, semantic-admission policy, or concrete local tool implementations |
...
owns the generic remote protocol. `backend/native` composes it with the frozen
tool registry and exact semantic replay; the process host does not yet select
that backend. The connector itself still does not own semantic Activities or
execute tools. The
