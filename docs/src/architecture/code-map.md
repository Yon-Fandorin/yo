...
| [`model_service`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/model_service/mod.rs) | Stable Provider, Account, and Model identities; explicit API-dialect selection with exactly one derived built-in connector; normalized HTTPS endpoints; Provider-and-Account base-profile plus whole-field model-override resolution into one complete typed profile; one complete-binding value and closed durable decoder shared by startup and native resume; a shared YAML scalar-style guard for structured profile numbers; Provider-and-Account-scoped catalog, context-profile, and credential resolution; manual/managed complete-binding composition with retained provenance and typed `BindingConflict`; an injected tokenizer-counting port; redacted resolved credentials; the secure bounded local `credentials.yaml` exact-pair CAS with private revisions; typed managed accounts and complete bindings in the bounded mode-`0600` `connections.yaml` old-or-new CAS; and a secret-free bounded `connection-operation.yaml` intent repository with closed durable phases, candidate-only multi-binding verification, public-first disconnect execution, a pure exact-state recovery table, and a same-directory local executor that commits journal, credential, and public phases under one retained operation lock | Connector wire translation, command-level target and confirmation presentation, or the process host's configuration-path selection |
