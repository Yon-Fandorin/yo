...
- service catalogs, discovery, and account capacity live in `yo-provider-*`; wire grammar and private replay live in `yo-connector-*`;
...
| [`yo-test-support`](https://github.com/Yon-Fandorin/yo/blob/develop/shared/yo-test-support/src/lib.rs) | Dev-only bounded local TLS fixtures and child diagnostics | Production networking or Provider grammar |
...
| [`providers/kimi/catalog.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/providers/kimi/src/catalog.rs), [`providers/kimi/catalog`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/providers/kimi/src/catalog) | Separate Platform AI and Code Membership catalog profiles and endpoints; one bounded authenticated Kimi `/models` snapshot; separate exact-once bounded Code Membership `/me` account-level and `/usages` account-capacity snapshots; first-valid duplicate handling; product-scoped reviewed exact-model overlays; complete selectable bindings; deterministic order; and visible typed reasons for every retained unavailable row | Kimi request/stream translation, private Session replay, Platform account capacity, or CLI consent and presentation |
| [`providers/openrouter`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/providers/openrouter/src/lib.rs), [`providers/qwencloud`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/providers/qwencloud/src/lib.rs) | Service-specific catalog reconstruction and validation; bounded OpenRouter discovery, normalization, and availability; QwenCloud catalog and browser-session account-capacity HTTP and decoding | CLI input, retry orchestration, credential CAS, core storage schema, or connector wire grammar |
| [`model_binding_admission.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/model_binding_admission.rs), [`model_profile_admission.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/model_profile_admission.rs) | The mandatory, secret-free `ModelBindingAdmission` port and neutral reasoning/tool/replay outcomes and typed admitted Chat image wire policy; generic OpenAI-dialect admission explicitly rejects Kimi. External connection preparation and managed construction consume the injected validator without a default or fallback | Kimi compatibility policy, authored profile resolution, Provider wire serialization, connector transport, or durable binding identity |
...
| [`connectors/kimi`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/connectors/kimi/src/lib.rs) | The concrete Kimi Chat Completions implementation of the neutral Connector port. One canonical Platform/Code matrix serves public secret-free binding admission and direct connector construction. The crate owns Kimi request serialization, stream grammar including exact cache-read usage availability, provider-private assistant codec and visible projection, and exact replay-size accounting; `yo-cli` selects it only for the exact Kimi identity and dialect | Catalog discovery, durable opaque-envelope storage, shared transport bytes, or managed semantic Activities |
...
| Module | Owns | Follow next |
|---|---|---|
| [`runner`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner.rs) | Public live-session facade, single terminal-owning loop, required readiness for every live source, indefinite idle waiting, configurable 120/60fps frame coalescing, a retained Inline Chat publication cursor, final cleanup reporting, and terminal-independent archived Chat, Transcript, and Request projection | `runner/state.rs` for the shared state/data facade and `runner/state/{input,commands,interview,observation,requests}.rs` for input and follow-up handling, slash commands and Session transitions, interview working copies, Journal/Chat observation, and request correlation; `runner/attention.rs` for opt-in live attention identities and dedupe; `runner/publication.rs` for persistent-row preparation and compact live size; `runner/frame.rs` for frame-rate policy; `runner/archival.rs` for stored output; `runner/unix.rs` and `runner/unix/presenter.rs` for live orchestration, post-flush geometry observation, owner-thread BEL emission, and visible motion scheduling |
| [`runner/state.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/state.rs) | Thin `TuiState` facade for `StateEffect`/`StateError`, `TuiState`/`PendingRequest` data, construction and accessors, shared frame commits, and cross-cutting coordination | `runner/state/input.rs`, `runner/state/commands.rs`, `runner/state/interview.rs`, `runner/state/observation.rs`, and `runner/state/requests.rs` for focused state behavior; `runner/state/image.rs`, `runner/state/presentation.rs`, and `runner/state/preview.rs` retain image, frame, and preview ownership |
| [`runner/session.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/session.rs), [`runner/session/metadata.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/session/metadata.rs), [`runner/session/construction.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/session/construction.rs), [`runner/session/continuation.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/session/continuation.rs), [`runner/session/bridges.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/session/bridges.rs), [`runner/session/generation.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/session/generation.rs) | The `TuiSession` facade retains terminal-independent state; `metadata.rs` owns resume entries, host status/document metadata, and bounded display forms; `construction.rs` owns constructors and session-local host/rendering configuration; `continuation.rs` owns inherited history, resume/session-tree/fork pickers, and new-session/model transitions; `bridges.rs` owns interview, image-preparation, status-line, and appearance bridges; `generation.rs` owns runner-facing session parts and publication-recovery evidence | `runner/state.rs` for shared retained state and frame commits; `runner/unix.rs` for terminal-generation resources; `yo-core` for semantic Session and continuation meaning |
| [`runner/state/input.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/state/input.rs), [`runner/state/commands.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/state/commands.rs), [`runner/state/interview.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/state/interview.rs), [`runner/state/observation.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/state/observation.rs), [`runner/state/requests.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/state/requests.rs) | `input.rs` owns input priority, editor submission, follow-up queueing, and request-bound secret input; `commands.rs` owns slash commands and Session transitions; `interview.rs` owns contextual draft handling and older vault maintenance; `observation.rs` owns Journal, durability, reference, and Chat observation; `requests.rs` owns request overlays and question/approval response correlation | Return to `runner/state.rs` for shared data and frame/publication commits; follow semantic Session, interview, or request meaning into `yo-core` |
| [`runner/state/history.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/state/history.rs) | Bounded accepted-prompt recall for the current Session, resume seeding from committed ordinary turns, filter panel, and draft preservation | `runner/state/input.rs` for admission outcome and input priority; `runner/session/continuation.rs` and `yo-cli/runtime/startup` for the bounded resume carrier; `prompt/assist.rs` for typed reference and image restoration |
| [`runner/state/find.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/state/find.rs) | Bounded, read-only search of finalized ordinary Chat messages, result-picker ownership, and preservation of the unsent draft | `runner/state/input.rs` for input priority; `runner/view.rs` and `transcript/viewport.rs` for semantic item navigation |
| [`runner/session/external_editor.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/session/external_editor.rs), [`runner/state/external_editor.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/state/external_editor.rs) | Terminal-independent external-editor snapshot, ordinary-draft eligibility, stale-result rejection, and typed reference/image preservation on import | `runner/unix.rs` for restored-terminal handoff; `yo-cli/execution/process/external_editor.rs` for the private file and child process |
| [`runner/chat.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/chat.rs) | Committed Chat projection and selection of the last completed original assistant answer for `/copy` | `runner/state/commands.rs` for command admission; `terminal/clipboard.rs` for the bounded terminal write |
...
| [`command`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/command.rs) | One child module per built-in slash command owning its ID, invocation, description, and typed effect; shallow registry composition and uniqueness validation; first-token query scanning; filtered prompt-overlay snapshots; exact-draft Esc ownership; and registry-derived help projection | `runner/state/input.rs` for input priority and submission; `runner/state/commands.rs` for slash-command effects and Session transitions; `overlay` for selection and presentation; `runner/model.rs` for `/model` resolution |
...
| [`terminal/clipboard.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/terminal/clipboard.rs) | Bounded OSC 52 encoding and unbuffered clipboard-request write | `runner/unix.rs` owns the output generation and user-visible send result |
...
[`tools/xtask/src/lib.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/tools/xtask/src/lib.rs)
is the crate's module-registration and re-export facade; it declares the workflow
modules and re-exports `xtask::run`. [`tools/xtask/src/cli.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/tools/xtask/src/cli.rs)
owns top-level CLI dispatch, argument routing, and usage, while its
`cli/check.rs`, `cli/docs.rs`, `cli/slice.rs`, and `cli/slice_contract.rs` children
own command-family parsing and adapters. The surrounding xtask modules own
structured checks that maintain this repository rather than the `yo` product.
Those checks classify changed paths and commit trailers for Slice review and
Developer Docs impact, and verify that Rust tests carry nearby explanatory
