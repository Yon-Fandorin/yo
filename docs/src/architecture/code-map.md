...
| [`model_service`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/model_service/mod.rs) | Stable Provider, Account, and Model identities; exact connector and API-dialect selection; normalized HTTPS endpoints; Provider-and-Account-scoped catalog, context-profile, and credential resolution; an injected tokenizer-counting port; redacted resolved credentials; and the secure, bounded, startup-only local `credentials.yaml` snapshot | A connector for request translation and the process host for configuration-path selection |
