...
| [`model_connector`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/model_connector/mod.rs) | The public provider-neutral Connector and stream ports plus shared request, observation, cancellation, failure, and limit vocabulary. OpenAI Responses wire construction and decoding no longer live in core; the still in-core Chat Completions and Kimi implementations remain transitional until their own extraction Slices | Provider wire grammar or a Yo-managed backend's semantic Activities and tool loop |
