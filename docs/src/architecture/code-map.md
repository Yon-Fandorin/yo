...
| [`model_service`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/model_service/mod.rs), [`model_service/openrouter_discovery.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/model_service/openrouter_discovery.rs), [`model_service/qwencloud_catalog.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/model_service/qwencloud_catalog.rs) | Stable Provider, Account, and Model identities; explicit API-dialect selection with exactly one derived built-in connector; normalized HTTPS endpoints; Provider-and-Account base-profile plus whole-field model-override resolution into one complete typed profile; bounded authenticated OpenRouter account-catalog transport and capability admission that retains every valid Model ID, narrows local-tools to no-tools when required, and exposes typed availability reasons; a closed release-known QwenCloud plan catalog with exact endpoints, complete selectable bindings, and retained unavailable rows whose reasons describe Yo runtime support rather than account entitlement; one complete-binding value and closed durable decoder shared by startup and native resume; current pre-version `connections.yaml`, `credentials.yaml`, and connection-operation shapes decoded through `yo-yaml`; Provider-and-Account-scoped catalog, context-profile, and credential resolution; manual/managed complete-binding composition with retained provenance and typed `BindingConflict`; an injected tokenizer-counting port; redacted resolved credentials; the secure bounded local `credentials.yaml` exact-pair CAS with private revisions; typed managed accounts and complete bindings in the bounded mode-`0600` `connections.yaml` old-or-new CAS; and a secret-free bounded `connection-operation.yaml` intent repository with closed durable phases, candidate-only multi-binding verification, public-first disconnect execution, a pure exact-state recovery table, and a same-directory local executor that commits journal, credential, and public phases under one retained operation lock | General Provider discovery, Connector wire translation, command-level target and confirmation presentation, or the process host's configuration-path selection |
...
| [`model_profile_admission.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/model_profile_admission.rs) | One shared admission of the resolved profile fields and local-tools/no-tools policy currently executable by native runtime and external connection verification | Authored profile resolution, connector transport, or durable binding identity |
| [`model_connector`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/model_connector/mod.rs) | Provider-neutral `openai-responses` and `openai-chat-completions` request serialization plus the product-scoped closed `kimi-chat-completions` Platform and Code K3/K2.7/K2.6 envelopes; a redacted opaque cache-affinity request hint serialized only by Code; request-local enabled/disabled tool exposure; exact Kimi private-assistant reconstruction; same-origin bounded redirects; bearer-authenticated HTTPS; cancellable request workers; dialect-specific bounded SSE framing, correlation, finish/terminal state, usage, and Kimi private-assistant emission | A Yo-managed backend for semantic Activities and the tool loop |
...
| [`backend/native`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/backend/native/mod.rs) | The provider-neutral Yo-managed model loop over connector-neutral observations, semantic model/tool Activities, serial validation-admission-approval-execution ordering, per-request context admission, bounded model rounds, visible refusal replay, typed Kimi private-assistant replay that is hidden from frontend observations and reused only by the matching connector, one opaque Session-stable cache-affinity hint attached without a Provider branch, exact per-response binding and usage attribution, cancellation cleanup, bounded replay deltas, and completed non-resumable exhaustion with binding latching | Startup model selection, tokenizer implementations, semantic-admission policy, or concrete local tool implementations |
