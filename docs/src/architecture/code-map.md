...
| [`runner/state/history.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-tui/src/runner/state/history.rs) | Bounded, current-session accepted-prompt recall, filter panel, and draft preservation | `runner/state/input.rs` for admission outcome and input priority; `prompt/assist.rs` for typed reference and image restoration |
