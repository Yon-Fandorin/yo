...
| [`model_service`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/model_service/mod.rs) | Stable Provider, Account, and Model identities; explicit API-dialect selection with exactly one derived built-in connector; normalized HTTPS endpoints; Provider-and-Account-scoped catalog, context-profile, and credential resolution; an injected tokenizer-counting port; redacted resolved credentials; the secure bounded local `credentials.yaml` capture plus exact-pair add, replace, and remove CAS with private revisions; and the bounded mode-`0600` `connections.yaml` snapshot, opaque revision, exact prepared preference mutation, operation lock, and old-or-new CAS | A connector for request translation, credential-and-public recovery orchestration, command orchestration, or the process host's configuration-path selection |
