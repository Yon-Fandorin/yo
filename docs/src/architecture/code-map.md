...
| [`src/connection.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/connection.rs), [`src/connection/external.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/connection/external.rs), [`src/connection/disconnect.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/connection/disconnect.rs), [`src/connection/input.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/connection/input.rs), [`src/connection/presentation.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/connection/presentation.rs) | Orchestrating `yo default`, Local Codex connect, exact configured external-model connect, and one-target managed disconnect under one shared recovery lane; prospective manual/managed composition; decision-first connect and disconnect previews with exact secondary details and controlling-TTY-width wrapping; controlling-TTY target, confirmation, and no-echo credential input; final config guard; first-success winner preservation; and startup capture of the stored preference | Free-form Provider onboarding, a general CLI widget framework, or physical repository storage |
