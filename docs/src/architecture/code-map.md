...
| [`src/connection.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/connection.rs), [`src/connection/external.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/connection/external.rs), [`src/connection/input.rs`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-cli/src/connection/input.rs) | Orchestrating `yo default`, Local Codex connect, and exact configured external-model connect under one shared recovery lane; prospective manual/managed composition; one confirmation for the complete same-account verification set; controlling-TTY no-echo credential input; final config guard; first-success winner preservation; and startup capture of the stored preference | Free-form Provider onboarding, external disconnect selection, or physical repository storage |
