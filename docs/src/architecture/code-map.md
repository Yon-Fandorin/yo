...
| [`model_connector`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/model_connector/mod.rs) | The provider-neutral Connector request, observation, cancellation, and limit vocabulary. During the staged extraction the existing in-core concrete Connectors remain the product runtime owner until the later composition Slice introduces the final port and switches routing | Provider wire ownership after composition, or a Yo-managed backend for semantic Activities and the tool loop |
...
| [`connectors/openai-responses`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/connectors/openai-responses/src/lib.rs) | Independently tested OpenAI Responses request serialization and bounded response-event decoding; introduced without changing current product routing and attached to the final neutral port by the later composition Slice | Model selection, credentials, shared transport bytes, or managed semantic Activities |
