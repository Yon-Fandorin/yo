...
| [`model_service`](https://github.com/Yon-Fandorin/yo/blob/develop/crates/yo-core/src/model_service/mod.rs) | Stable Provider, Account, and Model identities; explicit API-dialect selection with exactly one derived built-in connector; normalized HTTPS endpoints; closed `yo.binding-profile/v1` construction with exact versioned profile IDs, RFC 8785 parameter bytes, field framing, and SHA-256 digest; Provider-and-Account-scoped catalog, context-profile, and credential resolution; an injected tokenizer-counting port; redacted resolved credentials; the secure bounded local `credentials.yaml` exact-pair CAS with private revisions; the bounded mode-`0600` `connections.yaml` snapshot and old-or-new CAS; and a secret-free bounded `connection-operation.yaml` intent repository with closed durable phases, a pure exact-state recovery table, and a same-directory local executor that advances exact repository and journal phases under one retained operation lock | Durable binding-profile wire decoding, a connector for request translation, command-level connect/disconnect planning, managed-entry composition, verification and final configuration guarding, or the process host's configuration-path selection |
