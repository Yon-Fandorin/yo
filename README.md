...
`/status`는 현재 Session ID, 표시 중인 백엔드·모델, 작업 폴더, 실행 상태와 마지막으로
완료된 사용량 관측을 모델 호출 없이 보여준다. 사용량 관측은 세션 누적치가 아니며,
Session ID를 사용해 별도 터미널에서 `yo usage UUID`로 저장된 사용량을 확인할 수 있다.
...
개발용 오프라인 화면은 `cargo run --locked -p yo-tui --example chat_preview`로 연 뒤
`/preview`를 입력한다. 일반 Yo 대화에는 이 명령이 표시되거나 실행되지 않는다.
...
오프라인 화면의 대화로 돌아온다. 프리뷰 입력은 모델에 전송되지 않고 일반 대화에 저장되지 않는다.
...
이 예제는 모델에 연결하지 않으며 테스트 실행기의 시간 경고가 UI에 끼어들지 않는다.
...
채팅의 파일 변경 블록에 초점을 맞추고 `Alt+D`를 누르거나, 승인 패널에서
`Review proposed files`를 선택하면 상세 변경 화면이 열린다. 좌우 키로 파일을 바꾸고
위아래·휠로 diff 전체를 읽으며 F1로 대화에 돌아온다. 이 화면은 대화에서 관찰한 변경을
보여주며 Git 작업 트리 전체를 스캔하거나 파일을 수정하지 않는다. 상단에는 현재 파일
경로가 고정되며, 좁은 폭에서는
...
Codex `request_user_input`과 Grok `_x.ai/ask_user_question`의 일반 사용자 질문은 순서대로
제시한다. 두 번째 질문부터 `Shift+Tab`으로 이전 질문에
...
기록된 답변 수와 남은 질문을 요약한다. Esc는 Turn을 중단한다. Codex
`request_user_input`의 선택지·메모 없는 비밀 질문은 요청 전용 숨김 입력창에서 받는다.
현재 인터뷰 초안 v1은 선택한 Session과 작업 공간에서만 찾을 수 있다. 요청이 살아 있으면
`/interview continue`로 이어 쓰거나 `/interview discard`로 버린다. 요청이 끝났으면
`/interview view`로 읽거나 버릴 수 있으며, 제출이 확인되면 별도 초안은 삭제된다.
비밀 답안은 인터뷰 초안에 저장하지 않는다. 저장 제안이 있는 관리형 요청에서만 Yo가
`한 번만 사용`(기본값), `1~365일 저장`, `삭제할 때까지 저장`을 별도로 고르게 한다.
검증된 계정과 안전한 로컬 저장소가 있을 때 저장을 선택하면 암호화된 별도 저장소에 보관하고,
같은 대상의 다음 요청에서 `Ctrl-R`로 명시적으로 불러온 뒤 Enter로 제출할 수 있다. `/secrets`는 저장된 항목의 공개
정보를 보여주고 `/secrets delete <ID>`로 삭제한다. 위임형 비밀 질문과 일반 인터뷰
초안에는 이 저장 선택을 적용하지 않는다. `/preview`의 `plan`, `interview`, `changes` 예시에서 `Alt+D`로
모델 호출 없이 확인할 수 있다.
Grok 질문은 단일 선택, 직접 입력(`Other`), 선택 메모와 선택지 미리보기를 지원한다. 현재
공통 인터뷰 UI가 표현하지 못하는 Grok 다중 선택은 질문을 취소하고 명시적으로 실패하며,
Grok 질문을 비밀 입력으로 추정하지 않는다.

도구를 허용한 새 관리형 모델 세션은 예약 도구 `request_secret_input`으로 한 개의 비밀을
요청할 수 있다. 화면은 전송 대상 Provider·Model, 공개 목적, provider 보존 가능성과 제출 뒤
세션을 다시 사용할 수 없다는 점을 먼저 표시하고 값을 숨겨서 입력받는다. Yo는 비밀 값 대신
payload가 없는 제출 영수증만 내구성 있게 기록한 뒤 정확히 한 번의 최종 요청을 시작한다.
저장에 실패하면 전송하지 않으며, 제출한 세션은 이후 Turn·모델 교체·resume·fork를 허용하지
않는다. 최종 응답은 전부 받은 뒤 제출한 UTF-8 바이트열을 그대로 포함하면 전체를 숨긴다.
이 경로는 OpenAI Responses, OpenAI Chat Completions와 Kimi Chat Completions dialect에서
지원한다. 모델의 저장 제안은 사용자에게 저장 권한을 부여하지 않으며, 저장 대상 계정
확인이 불가능하면 일회성 입력만 제공한다.
...
거절한다. 선택 시 파일 내용을 읽거나 디렉터리를 재귀 첨부하지 않는다. 명시적으로
선택한 스킬은 실행 호스트에서 제출 직전에 신원·개정·사용 가능 여부를 다시 확인한 뒤
지침을 그 요청에 고정한다. 변경되거나 사라진 스킬은 초안을 보존하고 거절하며, 예약도
실제 전송 시 같은 검증을 따른다.

입력 편집에서 `Ctrl+A`·`Ctrl+E`는 실제 줄의 시작·끝으로 이동하고, `Ctrl+U`·`Ctrl+K`는
커서 앞·뒤를 줄 경계까지 삭제한다. 줄 경계에서 다시 삭제하면 이웃 줄을 연결한다.
초안이 있으면 ↑/↓는 화면에 보이는 줄을 따라 커서를 움직이며, 짧은 줄을 지난 뒤에도
원래 열로 돌아간다. 초안이 비어 있으면 대화 기록을 스크롤한다. 작성 중에도
PageUp/PageDown과 `Alt+↑/↓`로 기록을 읽을 수 있다.
...
`Ctrl+-`는 현재 일반 초안의 마지막 편집을 취소한다. 터미널에 따라 같은 키가 `Ctrl+7`로
전달되어도 동작한다. 연속 문자 입력은 단어 단위로, 붙여넣기·삭제·`Ctrl+C`의 초안 지우기는
각각 한 번에 되돌린다. 텍스트와 커서만 복원하고 삭제된 파일·스킬·이미지 선택 권한은
되살리지 않는다. 제출하거나 세션을 바꾸면 실행 취소 기록은 사라진다.
긴 일반 초안은 `Ctrl+G`로 외부 편집기에서 이어서 작성할 수 있다. `VISUAL`, 없으면
`EDITOR`에 설정한 명령을 실행하며, 편집기를 정상 종료하면 수정한 텍스트가 전송되지 않은
초안으로 돌아온다. 편집기를 실행할 수 없거나 편집이 실패하면 기존 초안을 유지한다.
GUI 편집기는 `VISUAL='code --wait'`처럼 파일을 닫을 때까지 기다리는 명령으로 설정한다.
일반 메시지가 실제로 승인된 뒤에는 `Ctrl+R`로 현재 세션의 최근 입력을 검색해 다시 편집할 수
있다. 입력창에 검색어를 쓰고 ↑/↓로 고른 뒤 Enter를 누르면 원래의 참조·이미지와 함께
초안으로 복원된다. 다시 Enter를 눌러야 전송된다. Esc는 검색 전 초안과 커서를 복원한다.
같은 Session을 재개할 때는 저장된 확정 `StartTurn`·`SteerTurn` 일반 입력도 이 bounded 이력에
복사해 검색할 수 있다. 현재 Session에 속한 기록만 선택하며 fork 부모의 입력은 섞지 않는다.
최대 32개·총 32 MiB를 메모리에 보관하고 별도 전역 history 파일은 사용하지 않으며, 세션을
바꾸거나 종료하면 사라진다. 비밀 입력과 질문 답변은 이 이력에 넣지 않는다. 예약 메시지를
꺼내는 `Alt+R`과는 별개다.
현재 대화의 메시지를 찾으려면 `Ctrl+F`를 누르거나 `/find 검색어`를 입력한다. 완료된 일반
사용자 메시지와 어시스턴트 답변만 최신순으로 검색하며 도구 출력·추론·공지·비밀 입력은
결과에 넣지 않는다. 검색어를 입력하고 ↑/↓로 결과를 고른 뒤 Enter를 누르면 해당 메시지로
이동한다. 이 동작은 새 메시지를 보내지 않으며, 열기 전 초안·커서·참조·이미지·실행 취소
기록을 유지한다. Esc나 `Ctrl+C`는 찾기만 닫는다. `End`를 누르면 최신 메시지로 돌아간다.
검색 범위가 상한에 닿으면 결과 창에 표시한다. `Ctrl+R`은 지난 입력을 초안으로 가져오는
별도 기능이다.
...
`/copy`는 현재 대화에서 답변이 있는 마지막 완료 턴의 텍스트 원문을 터미널 클립보드로 보낸다.
같은 턴의 이미지·리소스 블록은 제외하고, 텍스트가 전혀 없으면 복사할 수 없다고 알린다. 화면
줄바꿈·도구 출력·추론·상태 문구는 포함하지 않으며 모델 요청도 하지 않는다. 답변이 없거나
UTF-8 원문이 75,000바이트를 넘으면 이유를 표시한다. 성공 안내는 OSC 52 요청을 보냈다는
뜻이며, 터미널이나 tmux가 실제 클립보드 변경을 허용했는지는 붙여넣기로 확인해야 한다.
SSH·tmux에서는 연결된 터미널의 OSC 52 지원과 tmux의 `set-clipboard` 허용 설정이 필요하다.
...
실패 사유는 접지 않는다. `Ctrl+O`로 채팅에서 펼치거나 `Alt+D`로 파일 변경 상세 화면을 열 수 있다.
...
지원하지 않는 종류는 비활성화하며, 같은 턴의 정확한 미종료 파일 호출은 승인 패널의 변경 검토에 연결한다.
...
`approval-diff`는 승인 패널에서 연결된 파일의 상세 변경을 검토하는 예시다.
...
실제 `/help`는 현재 사용할 수 있는 명령 목록과 키보드 안내를 펼쳐진 문서로 보여준다.
명령 목록은 등록된 정의에서 생성하며 항목 탐색·접기·입력 편집·승인·인터뷰 조작을 함께 설명한다.
