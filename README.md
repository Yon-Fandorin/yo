...
모델에 이미지를 첨부하려면 입력창의 마지막 줄에 `/attach /절대/경로.png`를 입력한다.
Enter를 누르면 yo 실행 호스트의 로컬 PNG/JPEG를 준비하고 `[image]`로 바꾼다.
이 단계에서는 모델에 전송하지 않으며, 설명을 작성한 뒤 입력 전체를 제출한다.
...
현재 이미지 전송은 관리형 Kimi Code 연결에서 검토된 이미지 프로필을 명시한 경우 지원한다.
...
현재 첨부 입력은 `/attach PATH` 방식이다. 클립보드 이미지 붙여넣기는 아직 지원하지 않으며,
SSH에서는 경로가 yo를 실행하는 원격 호스트에 있어야 한다.
