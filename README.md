...
값은 기본적으로 프로세스 메모리에만 둔다. 정확한 Provider·Model·인증 계정 근거를 가진
delegated Codex 요청에서는 비밀을 입력한 뒤 `Ctrl+R`을 두 번 눌러 저장 위치를 확인하고
별도 암호화 복구를 선택할 수 있다. 재시작 뒤 같은 저장 복사본을 선택하고 목적지가 정확히
일치하는 새 비밀 요청을 받으면 `Ctrl+R`로 숨김 입력창에 복원하며, 별도의 Enter를 눌러야
전송된다. `Ctrl+F`는 복구 참조와 암호문을 제거한다. 복구 항목은 7일 뒤 만료되고, 공개
working-copy·Journal·로그에는 비밀 값·해시·길이를 기록하지 않는다. 이 로컬 암호화는
키와 vault를 함께 읽을 수 있는 백업·프로세스·계정에는 보호를 제공하지 않으며, Provider
보존이나 물리적 삭제를 보장하지 않는다. `/preview`의 `plan`, `interview`와 `changes`
입력 후 `/changes`로 모델 호출 없이 확인할 수 있다.
