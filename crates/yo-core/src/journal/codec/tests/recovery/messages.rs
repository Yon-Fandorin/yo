...
fn rejects_a_terminal_seal_that_does_not_match_durable_segments() {
    ...
    let mismatched = MessageEnded::new(
        activity,
        MessageStream::Agent,
    ...
    let commit = JournalCommit::incremental(sequenced(
        ...
        [
            ...
            JournalRecord::MessageEnded(MessageTerminal::new(None, mismatched)),
        ],
    ));
...
fn recovery_rejects_a_duplicate_terminal_seal() {
    ...
    let commit = JournalCommit::incremental(sequenced(
        1,
        [
            JournalRecord::MessageEnded(MessageTerminal::new(None, ended.clone())),
            JournalRecord::MessageEnded(MessageTerminal::new(None, ended)),
        ],
    ));
