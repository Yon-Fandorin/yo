...
use super::{
    ...
    model::{
        CatalogSource, ConnectionAccount, ConnectionCatalogSeed, ConnectionRevision,
        DecodedSnapshot, ModelLastFailure, ModelRequestFailureKind, StoredModelBinding,
        catalog::validate_catalog_seeds, validate_state,
    },
};
use crate::{
    AccountId, CompleteModelBinding, ConnectorId, EffectiveModelBinding, EffectiveModelProfile,
    HostId, ModelId, ModelProfileLayer, ModelProfileParameters, ModelSelection, NormalizedEndpoint,
    ProviderId, SEMANTIC_REPLAY_PROFILE, StartupTarget, VersionedProfileId,
};
...
fn parse_revision(
    ...
    ConnectionRevision::from_operation_journal(revision)
        .filter(|revision| !revision.is_absent())
