use super::*;
use crate::CompleteModelBinding;
...
#[test]
fn image_profile_round_trips_and_rejects_null_or_unknown_policy() {
    let (_directory, repository) = repository("image-profile");
    ...
    assert_eq!(repository.capture().unwrap().models(), &[binding]);
    let encoded = fs::read_to_string(repository.path()).unwrap();
    assert!(encoded.contains("image_input_profile: kimi-code-png-advisory/v1"));
