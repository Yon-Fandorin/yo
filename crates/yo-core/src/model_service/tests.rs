...
fn namespace_free_references_require_a_unique_coordinate_and_report_sorted_candidates() {
    ...
    let global = controller
        ...
        .unwrap_err()
        .to_string();
    let openrouter = global.find("Provider \"openrouter\"").unwrap();
    let qwen_default = global
        .find("Provider \"qwencloud\", Account \"default\"")
        .unwrap();
    let qwen_team = global
        .find("Provider \"qwencloud\", Account \"team\"")
        .unwrap();
    ...
    let provider = controller
        ...
        .unwrap_err()
        .to_string();
    assert!(provider.contains("Account \"default\""));
    assert!(provider.contains("Account \"team\""));
    ...
    assert!(absent.contains("Provider \"openrouter\", Account \"default\", Model \"same\""));
    assert!(absent.contains("Provider \"qwencloud\", Account \"default\", Model \"unique\""));
}
...
fn model_reference_preserves_vendor_punctuation_and_rejects_spelling_collisions() {
    ...
    assert!(collision.contains("Provider \"literal\""));
    assert!(collision.contains("Provider \"qwencloud\""));
}
...
fn model_reference_diagnostic_bounds_rejected_input() {
    ...
    assert!(
        diagnostic.contains("Provider \"qwencloud\", Account \"default\", Model \"qwen3.8-max\"")
    );
}
