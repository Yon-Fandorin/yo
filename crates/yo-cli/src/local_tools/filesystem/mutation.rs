...
use super::{
    FileIdentity, normalize_device_id, open_beneath, open_regular_file,
    ...
    let edits = edits
        ...
        .map(|edit| {
            ...
            Ok(ExactEdit {
                ...
                new_text: string(edit, "newText")?.to_owned(),
            })
        })
...
fn execute_edit_after_capture(
    ...
    let replacements = match plan_replacements(&original, &request.edits) {
    ...
    let planned_len = request
        ...
        .try_fold(original.len(), |length, edit| {
            length
                .checked_sub(edit.old_text.len())?
                .checked_add(edit.new_text.len())
        });
    if planned_len.is_none_or(|length| length > MAX_FILE_BYTES) {
        return mutation_error(request.path.display(), "too_large");
    }
    let planned = apply_replacements(&original, &request.edits, &replacements);
    if planned == original {
...
fn plan_replacements(original: &[u8], edits: &[ExactEdit]) -> Result<Vec<usize>, &'static str> {
...
fn apply_replacements(original: &[u8], edits: &[ExactEdit], starts: &[usize]) -> Vec<u8> {
...
mod tests {
    ...
    use super::{
        EditRequest, ExactEdit, MAX_FILE_BYTES, Scratch, Success, Terminal, UnwindCleanup,
        WriteRequest, apply_replacements, catch_failure, execute_edit_after_capture, execute_write,
        execute_write_after_mode, lock_mutation, overlapping_matches, plan_replacements,
        publish_in_parent,
    };
    use crate::local_tools::{filesystem::read::AdmittedPath, tests::TestDirectory};
    ...
    fn exact_edit_planning_detects_ambiguity_and_cross_edit_overlap() {
        assert_eq!(overlapping_matches(b"aaa", b"aa"), [0, 1]);
        assert_eq!(
