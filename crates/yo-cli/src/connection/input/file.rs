...
use crate::{AppError, connection::presentation::Confirmation};
...
pub(in crate::connection) struct AuthorizedCredentialFileInput {
    path: PathBuf,
}

impl AuthorizedCredentialFileInput {
    pub(in crate::connection) fn new(path: PathBuf) -> Self {
        Self { path }
    }
}
