...
use crate::command::account::domain::{
    AccountCapacityRecord, AccountCapacityReport, AccountProviderData, AccountQuery,
    AccountRefreshFailure, AccountTarget, local_host_label,
};
...
pub(super) fn render(records: &[AccountCapacityRecord]) -> Result<String, serde_json::Error> {
    let query = match records {
        [record] => match &record.target {
            AccountTarget::Exact(coordinate) => AccountQuery::Exact(coordinate.clone()),
            AccountTarget::LocalHost { provider } => AccountQuery::Provider(provider.clone()),
        },
        _ => AccountQuery::All,
    };
...
pub(in crate::command::account) fn render_for_query(
...
struct AccountCapacityOutput<'a> {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    provider_data: Option<&'a AccountProviderData>,
...
fn public_account_label<'a>(record: &'a AccountCapacityRecord) -> Cow<'a, str> {
    ...
    match &record.target {
        AccountTarget::Exact(coordinate) => Cow::Borrowed(coordinate.account.as_str()),
        AccountTarget::LocalHost { provider } => Cow::Owned(local_host_label(provider)),
    }
}
...
mod tests {
    ...
    use super::*;
    use crate::command::account::domain::{AccountCoordinate, AccountTarget};
