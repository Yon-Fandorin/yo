...
pub(crate) struct PickerTerminalScope {
    terminal: File,
...
impl PickerTerminalScope {
    pub(crate) fn enter(terminal: &File) -> Result<Self, AppError> {
...
impl<'a> PickerInput<'a> {
    pub(crate) const fn new(terminal: &'a File) -> Self {
        Self {
            terminal,
            pending: None,
    ...
    pub(crate) fn read_key(&mut self) -> Result<PickerKey, AppError> {
