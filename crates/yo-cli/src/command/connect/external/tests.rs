...
fn plan_includes_every_retained_binding_for_the_account() {
    ...
    let selected = catalog
        ...
        .unwrap();
    let plan =
        ExternalConnectPlan::prepare(&snapshot, &selection, selected, &StartupPolicy::initial())
            .unwrap();
...
fn plan_enforces_target_policy_before_confirmation() {
    ...
    let selected = catalog
        .resolve_model(selection.provider(), selection.account(), selection.model())
        .unwrap();
    let enforced_host = StartupPolicy::new(false, Some(StartupTarget::host_codex()), None).unwrap();
