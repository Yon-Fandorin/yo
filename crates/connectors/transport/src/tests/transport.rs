...
use yo_core::{ConnectorFailureKind, ModelConnectorLimits, ModelRequestFailureKind};

use super::super::{
    failure::{
        cancelled_failure, http_status_failure, limit_failure, protocol_failure, transport_failure,
    },
    http::{Origin, http_client_with_test_root, validate_redirect},
};
...
fn http_failures_preserve_only_the_status_code() {
    for (status, expected) in [
        ...
        (StatusCode::FORBIDDEN, ModelRequestFailureKind::AccessDenied),
        (
...
fn connector_failure_kinds_have_closed_request_observation_mapping() {
    assert_eq!(
        ...
        Some(ModelRequestFailureKind::Transport)
    );
    assert_eq!(
        protocol_failure("private-body-sentinel").request_failure_kind(),
        Some(ModelRequestFailureKind::Protocol)
    );
    assert_eq!(
        limit_failure("private-body-sentinel").request_failure_kind(),
        Some(ModelRequestFailureKind::ResponseLimit)
    );
