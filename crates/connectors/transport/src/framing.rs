...
impl SseFramer {
    pub fn new(maximum_event_bytes: usize, maximum_events: usize, protocol: &'static str) -> Self {
        Self {
            buffer: Vec::new(),
            event_count: 0,
            maximum_event_bytes,
    ...
    pub fn push(&mut self, bytes: &[u8]) -> SseFrameBatch {
    ...
    pub fn finish(&mut self) -> Result<Option<SseFrame>, ConnectorError> {
        self.buffer.clear();
        Ok(None)
    }
...
mod tests {
    ...
    // 빈 줄로 끝난 SSE event만 dispatch하고 EOF까지 event 구분자가 오지 않은 꼬리는
    // 완성된 frame으로 만들지 않아 잘린 terminal payload가 성공으로 보이지 않게 한다.
    #[test]
    fn discards_an_unterminated_tail_at_eof() {
        let mut framer = SseFramer::new(1_024, 8, "test");

        let batch = framer.push(b"data: complete\n\ndata: incomplete");

        assert_eq!(batch.frames.len(), 1);
        assert_eq!(batch.frames[0].data.as_deref(), Some("complete"));
