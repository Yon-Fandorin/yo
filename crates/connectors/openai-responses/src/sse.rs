...
impl ResponsesSseDecoder {
    ...
    fn refusal_delta(&mut self, event: &Value) -> Result<Vec<ModelConnectorEvent>, ConnectorError> {
        let output_index = usize_at(event, &["output_index"], "output index")?;
        let content_index = usize_at(event, &["content_index"], "content index")?;
        let item_id = string_at(event, &["item_id"], "output item id")?.to_owned();
    ...
    fn refusal_done(&mut self, event: &Value) -> Result<Vec<ModelConnectorEvent>, ConnectorError> {
        let output_index = usize_at(event, &["output_index"], "output index")?;
        let content_index = usize_at(event, &["content_index"], "content index")?;
        let item_id = string_at(event, &["item_id"], "output item id")?;
...
fn responses_incomplete_failure(reason: Option<&str>) -> yo_core::ModelRequestFailureKind {
    match reason {
...
fn responses_failed_failure(code: Option<&str>) -> yo_core::ModelRequestFailureKind {
    match code {
