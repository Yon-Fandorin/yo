...
fn final_request_cap_and_pressure_use_one_complete_image_estimate() {
    ...
    let openrouter = CompleteModelBinding::from_durable_json(include_str!(concat!(
        ...
        for image_count in [0, 1, 2] {
            ...
            let mut backend = NativeModelBackend::with_connector_and_profile(
                ...
                NativeModelBackendServices::new(
                    Box::new(|complete: &CompleteModelBinding| {
                        if complete.profile().replay_profile().as_str() == "semantic-only/v1" {
                            yo_core::admit_standard_complete_binding(complete)
                        } else {
                            kimi_admission(complete)
                        }
                    }),
                    None,
            ...
            let reserve = if image_count == 0 { 0 } else { 1024 };
            {
                ...
                assert_eq!(
                    ...
                    Some(complete.profile().context().input_token_limit() - estimate - reserve)
                );
