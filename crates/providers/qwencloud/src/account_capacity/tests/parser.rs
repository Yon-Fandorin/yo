...
use super::{
    super::{
        decode::{decode_gateway_envelope, decode_snapshot, extract_sec_token},
        http::{LOGIN_COOKIE, cookie_value},
        validate_account_session,
    },
    account, gateway_payload, provider,
};
...
fn decodes_personal_token_plan_windows_and_dynamic_tier() {
    let usage = json!({ "per5HourPercentage": 0.251, "per5HourResetTime": 1_800_000_000_000_i64, "per1WeekPercentage": 0.55, "per1WeekResetTime": 1_800_500_000_000_i64 });
    let subscription = json!({ "specCode": "moderato" });
    let quota_config = json!({ "moderato": { "five_hour": 3_000, "weekly": 10_000 } });
...
fn accepts_weekly_window_when_five_hour_window_is_omitted() {
    ...
    let subscription = json!({ "specCode": "lite" });
    let quota_config = json!({ "lite": { "weekly": 2_500 } });
    let (snapshot, provider_data) = decode_snapshot(
        &usage,
        &subscription,
        &quota_config,
...
fn unwraps_only_successful_gateway_envelopes() {
    let payload = json!({ "specCode": "moderato" });
    assert_eq!(
...
fn rejects_invalid_percentage_or_missing_active_tier() {
    ...
    assert!(
        decode_snapshot(
            &json!({ "per1WeekPercentage": 1.1, "per1WeekResetTime": 1_800_500_000_i64 }),
            &subscription,
            &quota_config,
            &provider(),
            &account()
    ...
    assert!(
        decode_snapshot(
            &json!({ "per1WeekPercentage": 0.5, "per1WeekResetTime": 1_800_500_000_i64 }),
            &subscription,
            &json!({}),
            &provider(),
            &account()
