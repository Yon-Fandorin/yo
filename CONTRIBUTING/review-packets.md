...
```json
{
  "schema": "yo.slice-review-prepare-request/v1alpha7",
  "slice": "example-slice",
...
Preparations from v1alpha2 through v1alpha7 append one fixed output-contract
...
emits its same-numbered result schema: v1alpha1 through v1alpha7 remain
...
execution profile and Usage-bound delivery shape. New v1alpha7 preparation
uses v1alpha5 admission for `host:grok`, which selects either its native
read-only profile or the bounded Yo-owned outer sandbox with EOF stdin before
packet construction. It permits no prompt, ACP initialize, Session, or
Provider request. `host:codex` retains v1alpha3 state-ready admission until an
equivalent request-free isolation probe is defined. A managed v1alpha7 target
uses v1alpha6 admission, so a typed blocking failure at least 5 hours old is a
stale warning and the already-authorized exact delivery becomes its sole
revalidation instead of requiring a probe or manual state edit. Callers do not
supply Provider or Account coordinates for a delegated host.
...
For a new review, v1alpha7 preserves v1alpha5's authority routing and
v1alpha4's Usage-bound delivery while selecting the current managed-freshness
and Grok execution-isolation admissions. Frozen v1alpha6 retains the native
Grok execution-profile probe and permanent managed-failure blocking behavior.
...
`yo.slice-review-prepare-result/v1alpha7`. Frozen v1alpha4 keeps
